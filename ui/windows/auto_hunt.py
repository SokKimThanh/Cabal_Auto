    ...
    if "monster_selected_name" in cfg and cfg["monster_selected_name"]:
        if not cfg.get("monster_rotation"):
            cfg["monster_rotation"] = [
                {"name": cfg["monster_selected_name"], "priority": 1, "enabled": True}
            ]
...
def get_monster_rotation_targets(cfg):
    ...
    for monster in enabled_monsters:
        ...
        for t in all_templates:
            ...
            if (
                name_clean in tmpl_clean
                or tmpl_clean in name_clean
                or tmpl_clean.startswith(name_clean)
                or name_clean.startswith(tmpl_clean)
            ):
                monster_templates.append(t)

        if monster_templates:
            result.append(
                {"name": name, "priority": priority, "templates": monster_templates}
            )
    ...
    for monster in monster_targets:
        ...
        for tmpl in templates:
            ...
            if region_strategy == "custom" and tmpl.get("region"):
                ...
                region = (
                    window_bounds.get("left", 0),
                    window_bounds.get("top", 0),
...
def main():
    ...
    if skills_path.exists():
        try:
            ...
            print(
                f"Skill runtime initialized: {len(skill_runtime.attack_skills)} attacks, {len(skill_runtime.buff_skills)} buffs"
            )
    ...
    if use_rotation:
        training_info = (
            " [TRAINING MODE]" if training_mode_enabled and is_training_dummy else ""
        )
        print(
            f"Monster rotation enabled: {rotation_mode} mode, {len(monster_targets)} monsters{training_info}"
        )
        for idx, monster in enumerate(monster_targets):
            prefix = (
                "→"
                if idx == current_monster_index and rotation_mode == "sequence"
                else " "
            )
            dummy_mark = (
        ...
        while True:
            ...
            if now - last_search >= search_interval:
                ...
                if use_rotation:
                    ...
                    if rotation_mode == "sequence":
                        ...
                        search_order = [
                            monster_targets[
                                current_monster_index % len(monster_targets)
                            ]
                        ]
                    else:  # priority mode
                    ...
                    box, match_info, monster_name = locate_monster_target(
                        search_order, window_bounds
                    )
                ...
                if have_target and match_info:
                    ...
                    if last_match_info != match_info:
                        ...
                        template_name = match_info.get("name") or match_info.get(
                            "path", "unknown"
                        )
                        threshold = match_info.get("threshold", 0.8)
                        confidence = match_info.get("confidence", 0.0)
                        log_monster_name = match_info.get(
                            "monster_name", monster_name or ""
                        )
                        logger.log_match(
                            template_name, box, threshold, confidence, log_monster_name
                        )

                        rotation_info = f" [{rotation_mode}]" if use_rotation else ""
                        monster_info = (
                            f" Monster: {log_monster_name}" if log_monster_name else ""
                        )
                        print(
                            f"[Match{rotation_info}]{monster_info} Template: {template_name}, "
                    ...
                    if time_target_lost and (
                        now - time_target_lost >= lost_timeout_sec
                    ):
                        # State transition: attack -> search
                        if state == "attack":
                            duration = (
                                now - attack_start_time if attack_start_time else 0
                            )

                            # Only transition if we've been attacking long enough
                            if duration >= attack_min_duration_sec:
                                if last_match_info and isinstance(
                                    last_match_info, dict
                                ):
                                    template_name = last_match_info.get(
                                        "name"
                                    ) or last_match_info.get("path", "unknown")
                                    log_monster_name = last_match_info.get(
                                        "monster_name", last_monster_name or ""
                                    )
                                else:
                                    template_name = last_monster_name or "unknown"
                                    log_monster_name = last_monster_name or ""
                                logger.log_lost(
                                    template_name, log_monster_name, duration
                                )
                                logger.log_state_change(
                                    "attack", "search", "target_lost"
                                )
                                state = "search"
                                attack_start_time = None

                                rotation_info = (
                                    f" [{rotation_mode}]" if use_rotation else ""
                                )
                                monster_info = (
                                    f" Monster: {log_monster_name}"
                                    if log_monster_name
                                    else ""
                                )
                                print(
                                    f"[Lost{rotation_info}]{monster_info} Target lost after {duration:.1f}s"
                                )

                                # Sprint 22 Patch 1: Skip rotation if in training mode with training dummy
                                should_rotate = (
                                    use_rotation and rotation_mode == "sequence"
                                )
                                if training_mode_enabled and is_training_dummy:
                                    should_rotate = False
                                    print(
                                        f"[Training Mode] Staying on training dummy - no target rotation"
                                    )
                                ...
                                if should_rotate:
                                    ...
                                    next_monster = monster_targets[
                                        current_monster_index
                                    ]["name"]

                                    # Update training dummy status for new monster
                                    is_training_dummy = (
                                        next_monster.get("training_mode", False)
                                        if isinstance(next_monster, dict)
                                        else monster_targets[current_monster_index].get(
                                            "training_mode", False
                                        )
                                    )

                                    print(
                                        f"[Rotation] Switching to: {next_monster} ({current_monster_index+1}/{len(monster_targets)})"
                                    )
            ...
            if skill_runtime:
                ...
                if buff_key:
                    ...
                    hold_time = (
                        buff_info.get_hold_time_ms() if buff_info else attack_press_ms
                    )
            ...
            if skill_runtime:
                ...
                if attack_key:
                    ...
                    hold_time = (
                        attack_info.get_hold_time_ms()
                        if attack_info
                        else attack_press_ms
                    )
