...
class OverlayWindowPyWin32:
    ...
    def __init__(
        ...
        self.target_rect = target_rect or {
            "left": 0,
            "top": 0,
            "width": 800,
            "height": 600,
    ...
    def create(self) -> None:
        ...
        )  # No decorations  # Visible by default
    ...
    def _render_gdi(
        self, hdc: int, detections: List[DetectionBox], width: int, height: int
    ) -> None:
        """
    ...
    def _draw_text_label(
        self, hdc: int, x: int, y: int, box_height: int, label: str, confidence: float
    ) -> None:
        """
        ...
        bg_rect = (x, text_y, x + text_width, text_y + TEXT_FONT_HEIGHT + TEXT_PADDING)
    ...
    def update_target_rect(self, rect: Dict[str, int]) -> None:
        ...
        if self.hwnd:
            ...
            size_changed = (
                old_rect is None
                or old_rect["width"] != rect["width"]
                or old_rect["height"] != rect["height"]
            )
    ...
    def set_alpha(self, alpha: float) -> None:
        ...
        if self.hwnd:
            ...
            win32gui.SetLayeredWindowAttributes(
                self.hwnd, 0, alpha_byte, win32con.LWA_ALPHA
            )
...
def create_detection_box(
    ...
    return DetectionBox(
        x=x, y=y, w=w, h=h, label=label, color=color, confidence=confidence
    )
