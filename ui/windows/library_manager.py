...
class LibraryManagerWindow(tk.Toplevel):
    ...
    def _on_template_region_change(self):
        ...
        if tmpl is None:
            return
        vals = {
            k: self.template_region_vars[k].get().strip()
    ...
    def _open_setup_wizard_vision(self):
        ...
        dialog.title(
            "Setup Wizard Vision" if self.lang == "en" else "Thiết Lập Vision Nâng Cao"
        )
...
"""
    ...
    def _pick_template_region(self):
        ...
        if not hasattr(self, "current_monster") or self.current_monster is None:
            messagebox.showwarning(
                ...
                (
                    "Please select a monster first."
                    if self.lang == "en"
                    else "Vui lòng chọn một quái trước."
                ),
            )
            return
    ...
    def _resolve_current_template_params(
        self,
    ) -> Optional[tuple[str, float, Optional[tuple[int, int, int, int]]]]:
        ...
        try:
            th = float(
                (self.template_threshold_var.get() or tmpl.get("threshold", 0.85))
            )
    ...
    def _test_template_recognition(self):
        ...
        if not params:
            messagebox.showinfo(
                "Info" if self.lang == "en" else "Thông báo",
                "No template selected" if self.lang == "en" else "Chưa chọn ảnh mẫu",
            )
            return
        ...
        if box:
            ...
            if conf is None:
                msg = (
                    f"Match found at ({x}, {y})"
                    if self.lang == "en"
                    else f"Tìm thấy tại ({x}, {y})"
                )
            else:
                msg = (
                    f"Match found at ({x}, {y}) - Confidence: {conf:.2f}"
                    if self.lang == "en"
                    else f"Tìm thấy tại ({x}, {y}) - Độ khớp: {conf:.2f}"
                )
            messagebox.showinfo("Test Recognition", msg)
        else:
            messagebox.showinfo(
                "Test Recognition",
                "No match found" if self.lang == "en" else "Không tìm thấy",
            )
    ...
    def _auto_detect_template_region(self):
        ...
        if not box:
            messagebox.showinfo(
                "Auto-Detect",
                (
                    "No match found to derive region"
                    if self.lang == "en"
                    else "Không tìm thấy ảnh để suy ra vùng"
                ),
            )
            return
        l, t, w, h = box
    ...
    def _unique_asset_name(
        self, monster_name: str, template_name: str, ext: str
    ) -> str:
    ...
    def _update_save_button_tooltip(self, has_unsaved: bool):
        ...
        try:
            ...
            tooltip_key = (
                "tip_apply_all_unsaved" if has_unsaved else "tip_apply_all_saved"
            )
            ...
            for m in self.monsters:
                ...
                for t in templates:
                    ...
                    if self._is_under(abs_p, self.tmp_capture_dir):
                        ...
                        dest_name = self._unique_asset_name(
                            m.get("name", ""), t.get("name", ""), abs_p.suffix
                        )
    ...
    class _RegionCaptureOverlay(tk.Toplevel):
        def __init__(
            self, parent, restrict_bbox: Optional[tuple[int, int, int, int]] = None
        ):
            super().__init__(parent)
            self.parent = parent
            self.withdraw()
            ...
            if self._restrict:
                ...
                self._guide = self.canvas.create_rectangle(
                    x0, y0, x1, y1, outline="#FFA000", width=2
                )
        ...
        def _on_drag(self, event):
            if self._start and self._rect:
                ...
                self._size_text = self.canvas.create_text(
                    x1 + 10, y1 + 10, text=label, fill="white", anchor="nw"
                )
    ...
    def _build_ui(self):
        ...
        self.notebook.add(
            self.rotation_tab,
            text="🎮 " + ("Skill Rotation" if self.lang == "en" else "Chu Kỳ Chiêu"),
        )
    ...
    def _build_monster_tab(self, parent: tk.Frame):
        ...
        self._make_icon_button(
            ...
            relief="flat",
            font=UI.FONT_BUTTON,
            padx=self.ui_btn_padx,
            pady=self.ui_btn_pady,
        ...
        if search_img:
            try:
                tk.Label(
                    search_container, image=search_img, text="", bg="#F5F5F5"
    ...
    def _add_monster_to_tree(self, monster: dict):
        ...
        if not self.monster_tree:
            return
        self.monster_tree.insert("", "end", text=f"  {name}", tags=("monster",))
    ...
    def _render_monster_edit_form(self, parent: tk.Frame):
        ...
        if not hasattr(self, "current_monster") or self.current_monster is None:
            return
        form = tk.Frame(
            parent, bg="#E3F2FD", highlightbackground="#2196F3", highlightthickness=2
        )
        ...
        self.mon_dmg_var = tk.StringVar(
            value=str(self.current_monster.get("damage_per_hit", 0))
        )
        self.mon_prio_var = tk.StringVar(
            value=str(self.current_monster.get("priority", 1))
        )
        ...
        tk.Entry(body, textvariable=self.mon_name_var, font=self.ui_font_text).grid(
            row=0, column=1, sticky="ew", pady=(0, 6)
        )

        tk.Label(body, text="HP", bg="#E3F2FD", font=self.ui_font_label).grid(
            row=1, column=0, sticky="w"
        )
        tk.Entry(
            body, textvariable=self.mon_hp_var, width=12, font=self.ui_font_text
        ).grid(row=1, column=1, sticky="w")

        tk.Label(
            body,
            text=("Damage" if self.lang == "en" else "Sát thương"),
            bg="#E3F2FD",
            font=self.ui_font_label,
        ).grid(row=1, column=2, sticky="w", padx=(12, 0))
        tk.Entry(
            body, textvariable=self.mon_dmg_var, width=12, font=self.ui_font_text
        ).grid(row=1, column=3, sticky="w")

        tk.Label(
            body,
            text=("Priority" if self.lang == "en" else "Ưu tiên"),
            bg="#E3F2FD",
            font=self.ui_font_label,
        ).grid(row=2, column=0, sticky="w", pady=(6, 0))
        tk.Entry(
            body, textvariable=self.mon_prio_var, width=8, font=self.ui_font_text
    ...
    def _add_monster(self):
        ...
        dialog = MonsterDialog(
            self, self.lang, mode="add", icon_helper=icon_helper, i18n_registry=i18n_t
        )
        ...
        if dialog.result:
            ...
            messagebox.showinfo(
                ...
                (
                    f"Monster '{dialog.result.get('name', 'Unknown')}' has been added."
                    if self.lang == "en"
    ...
    def _edit_monster(self):
        ...
        if not selection:
            messagebox.showwarning(
                ...
                (
                    "Please select a monster to edit."
                    if self.lang == "en"
                    else "Vui lòng chọn quái để sửa."
                ),
            )
            return
        ...
        if dialog.result:
            ...
            messagebox.showinfo(
                ...
                (
                    f"Monster '{dialog.result.get('name', 'Unknown')}' has been updated."
                    if self.lang == "en"
    ...
    def _delete_monster(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection" if self.lang == "en" else "Chưa Chọn",
                (
                    "Please select a monster to delete."
                    if self.lang == "en"
                    else "Vui lòng chọn quái để xóa."
                ),
            )
            return
        ...
        if response:
            ...
            messagebox.showinfo(
                ...
                (
                    f"Monster '{monster.get('name', 'Unknown')}' has been deleted."
                    if self.lang == "en"
    ...
    def _duplicate_monster(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection" if self.lang == "en" else "Chưa Chọn",
                (
                    "Please select a monster to duplicate."
                    if self.lang == "en"
                    else "Vui lòng chọn quái để nhân bản."
                ),
            )
            return
        ...
        messagebox.showinfo(
            ...
            (
                f"Monster '{original.get('name', 'Unknown')}' has been duplicated."
                if self.lang == "en"
    ...
    def _show_template_editor(self, monster: Optional[dict]):
        ...
        if monster is None:
            ...
            if self.template_list_panel:
                tk.Label(
                    self.template_list_panel,
                    text="👈",
                    font=(UIStyle.resolve_font_family("body"), 36),
                    bg="#FFFFFF",
                    fg="#DDD",
                ).pack(pady=(40, 6))
                tk.Label(
                    self.template_list_panel,
                    text=(
                        "Select a monster to view templates"
                        if self.lang == "en"
                        else "Chọn quái để xem template"
                    ),
                    bg="#FFFFFF",
                    fg="#999",
                ).pack()
        ...
        tk.Label(
            header_frame,
            text=f"{len(templates)} "
            + ("Templates" if self.lang == "en" else "Mẫu hình"),
            font=(UIStyle.resolve_font_family("body"), 10, "bold"),
            bg="#FFFFFF",
            fg="#424242",
        ).pack(side="left")
        ...
        self.template_tree.heading(
            "name", text="Template" if self.lang == "en" else "Template"
        )
        ...
        for idx, template in enumerate(templates):
            ...
            if image_obj is not None:
                self.template_tree.insert(
                    "", "end", text="", image=image_obj, values=(name,)
                )
            else:
        ...
        tk.Label(
            self.template_list_panel,
            text=(
                "Nhấp để chọn template. Kéo danh sách để xem thêm."
                if self.lang == "vi"
                else "Click to select a template. Scroll to see more."
            ),
            bg="#FFFFFF",
            fg="#757575",
            font=(UIStyle.resolve_font_family("body"), 8),
            anchor="w",
        ...
        self.template_preview_label = tk.Label(
            preview_frame, bg="#FAFAFA", relief="solid"
        )
        ...
        self._make_icon_button(
            ...
            "📸",
            "tip_capture_image",
            command=lambda: self._capture_into_path_var(window=False),
            font=UI.FONT_BUTTON,
            bg="#9C27B0",
            fg="#FFFFFF",
            relief="flat",
        ...
        hint_txt = (
            "Chọn một template để xem trước và sửa. Dùng Thêm/Sửa/Xóa ở góc phải."
            if self.lang == "vi"
            else "Select a template to preview and edit. Use Add/Edit/Delete on the top-right."
        )
        tk.Label(
            self.template_edit_panel,
            text=hint_txt,
            bg="#FFFFFF",
            fg="#757575",
            font=(UIStyle.resolve_font_family("body"), 8),
        ...
        self.template_form_frame = tk.Frame(
            ...
            relief="flat",
            borderwidth=0,
            highlightbackground=UI.COLOR_PRIMARY,
        ...
        self.template_name_var = tk.StringVar()
        self.template_name_entry = tk.Entry(
            name_col,
        ...
        tk.Label(
            name_col,
            text=(
                "Tên hiển thị của mẫu hình."
                if self.lang == "vi"
                else "Display name for this template."
            ),
            bg=UI.BG_SECTION,
            fg=UI.COLOR_HINT,
            font=UI.FONT_SMALL,
        ...
        self.template_threshold_var = tk.StringVar(value="0.85")
        self.template_threshold_entry = tk.Entry(
            th_input,
            textvariable=self.template_threshold_var,
            font=UI.FONT_TEXT,
            width=12,
            relief="solid",
            borderwidth=1,
        ...
        tk.Label(
            th_col,
            text=(
                "Gợi ý: 0.80 - 0.90. Cao hơn -> ít nhận nhầm, nhưng khó khớp."
                if self.lang == "vi"
                else "Tip: 0.80 - 0.90. Higher = less false positives, but harder to match."
            ),
            bg=UI.BG_SECTION,
            fg=UI.COLOR_HINT,
            font=UI.FONT_SMALL,
        ...
        # tk.Label(region_frame, text=('Region Override (L,T,W,H)' if self.lang=='en' else 'Vùng ghi đè (L,T,R,D)'), bg='#E3F2FD', font=(UIStyle.resolve_font_family('body'), 9, 'bold'), fg='#424242', anchor='w').pack(fill='x', pady=(0,4))
        ...
        #     entry = tk.Entry(region_inputs, textvariable=self.template_region_vars[key], width=6, font=(UIStyle.resolve_font_family('body'), 10), relief='solid', borderwidth=1, state='readonly')
        ...
        # ), bg='#E3F2FD', fg='#757575', font=(UIStyle.resolve_font_family('body'), 8), anchor='w').pack(fill='x', pady=(4,6))
        ...
        hint_text = (
            ...
            if self.lang == "vi"
            else "Click the button below to learn about upcoming feature upgrades"
        )
        tk.Label(
            wizard_frame,
            text=hint_text,
            bg="#E8F5E9",
            fg="#558B2F",
            font=(UIStyle.resolve_font_family("body"), 8, "italic"),
            anchor="w",
            wraplength=400,
        ...
        self._suspend_template_var_traces = False
        try:
            self.template_name_var.trace(
                "w", lambda *args: self._on_template_name_change()
            )
            self.template_path_var.trace(
                "w", lambda *args: self._on_template_path_change()
            )
            self.template_threshold_var.trace(
                "w", lambda *args: self._on_template_threshold_change()
            )
    ...
    def _on_template_tree_select(self, event):
        ...
        templates = (
            self.current_monster.get("templates", [])
            if hasattr(self, "current_monster")
            else []
        )
        ...
        try:
            ...
            try:
                l = (
                    str(int(region.get("left", "")))
                    if isinstance(region, dict)
                    and str(region.get("left", "")).strip() != ""
                    else ""
                )
                t = (
                    str(int(region.get("top", "")))
                    if isinstance(region, dict)
                    and str(region.get("top", "")).strip() != ""
                    else ""
                )
                w = (
                    str(int(region.get("width", "")))
                    if isinstance(region, dict)
                    and str(region.get("width", "")).strip() != ""
                    else ""
                )
                h = (
                    str(int(region.get("height", "")))
                    if isinstance(region, dict)
                    and str(region.get("height", "")).strip() != ""
                    else ""
                )
            except Exception:
                l = t = w = h = ""
    ...
    def _update_toggle_button_icon(
        self, icon_name: str, fallback: str, tooltip_key: str
    ):
    ...
    def _save_current_monster_inline(self):
        ...
        try:
            ...
            self.current_monster["description"] = self.mon_desc_text.get(
                "1.0", "end"
            ).strip()
        except Exception as e:
    ...
    def _delete_current_monster_inline(self):
        ...
        msg = (
            f"Delete monster '{name}'?" if self.lang == "en" else f"Xóa quái '{name}'?"
        )
    ...
    def _capture_into_path_var(self, window: bool = False):
        ...
        if capture_region_and_save is None:
            messagebox.showerror(
                "Error" if self.lang == "en" else "Lỗi", "Capture helper not available."
            )
            return
        ...
        try:
            ...
            temp_name = self._unique_asset_name(
                self.current_monster.get("name", ""), current_name, src.suffix
            )
        ...
        try:
            if (
                bbox
                and hasattr(self, "template_region_vars")
                and not any(v.get().strip() for v in self.template_region_vars.values())
            ):
                l, t, w, h = bbox
                self.template_region_vars["left"].set(str(l))
                self.template_region_vars["top"].set(str(t))
    ...
    def _update_template_preview(self, template: Optional[dict]):
        ...
        if not hasattr(self, "template_preview_label") or not hasattr(
    ...
    def _delete_template_inline(self):
        ...
        response = messagebox.askyesno(
            "Confirm Delete" if self.lang == "en" else "Xác Nhận Xóa",
            (
                f"Delete template '{template.get('name', 'Unknown')}'?"
                if self.lang == "en"
        ...
        if response:
            ...
            try:
                ...
                if img_path and not still_used:
                    ...
                    if self._is_under(abs_p, self.assets_mon_dir) or self._is_under(
                        abs_p, self.tmp_capture_dir
                    ):
                        if abs_p.exists():
                            abs_p.unlink()
    ...
    def _save_template_form(self):
        ...
        template_data = {"name": name, "path": path, "threshold": threshold}
        ...
        if self.template_form_mode == "add":
            ...
            templates = self.current_monster.get("templates", [])
            if (
                self.template_form_edit_index is not None
                and self.template_form_edit_index < len(templates)
            ):
    ...
    def _capture_template(self):
        ...
        messagebox.showinfo(
            ...
            (
                "Template captured and saved."
                if self.lang == "en"
                else "Đã chụp và lưu template."
            ),
        )
    ...
    def _capture_template_window(self):
        ...
        if not hwnd:
            messagebox.showwarning(
                ...
                (
                    "Cannot determine game window. Make it active or set it in setup."
                    if self.lang == "en"
                    else "Không xác định được cửa sổ game. Hãy kích hoạt cửa sổ game hoặc cấu hình trong wizard."
                ),
            )
            return
        ...
        if not rect:
            messagebox.showwarning(
                ...
                (
                    "Unable to get window bounds."
                    if self.lang == "en"
                    else "Không lấy được kích thước cửa sổ."
                ),
            )
            return
    ...
    def _build_skill_tab(self, parent: tk.Frame):
        ...
        type_combo = ttk.Combobox(
            filter_frame,
            textvariable=self.skill_type_filter_var,
            state="readonly",
            width=12,
        ...
        tk.Label(
            preview_bar,
            text=("Skill Image" if self.lang == "en" else "Hình Ảnh Kỹ Năng"),
            bg="#FFF3E0",
            fg="#E65100",
    ...
    def _show_skill_details(self, skill: Optional[dict]):
        ...
        if skill is None:
            if not self.skill_details_panel:
                return
            tk.Label(
                self.skill_details_panel,
                text="← "
                + (
                    "Select a skill to edit"
                    if self.lang == "en"
                    else "Chọn kỹ năng để sửa"
                ),
                bg="#FFFFFF",
                fg="#9E9E9E",
                font=self.ui_font_label,
    ...
    def _render_skill_edit_form(self, parent: tk.Frame):
        ...
        self.skill_type_var = tk.StringVar(
            value=self.current_skill.get("type", "attack")
        )
        self.skill_cooldown_var = tk.StringVar(
            value=str(self.current_skill.get("cooldown", 0.0))
        )
        self.skill_cast_time_var = tk.StringVar(
            value=str(self.current_skill.get("cast_time", 0.0))
        )
        ...
        tk.Entry(body, textvariable=self.skill_name_var, font=self.ui_font_text).grid(
            row=0, column=1, columnspan=3, sticky="ew", pady=(0, 6)
        )
        ...
        tk.Entry(
            body, textvariable=self.skill_key_var, width=8, font=self.ui_font_text
        ...
        tk.Label(
            ...
            bg="#E3F2FD",
            font=self.ui_font_label,
        ).grid(row=1, column=2, sticky="w", padx=(12, 0), pady=(0, 6))
        type_combo = ttk.Combobox(
            body,
        ...
        tk.Entry(
            body, textvariable=self.skill_cooldown_var, width=12, font=self.ui_font_text
        ...
        tk.Label(
            ...
            bg="#E3F2FD",
            font=self.ui_font_label,
        ).grid(row=2, column=2, sticky="w", padx=(12, 0), pady=(0, 6))
        tk.Entry(
            body,
            textvariable=self.skill_cast_time_var,
            width=12,
            font=self.ui_font_text,
    ...
    def _show_skill_image_preview(self, skill: Optional[dict]):
        ...
        if not image_path or not Path(image_path).exists():
            tk.Label(
                self.skill_preview_panel,
                text="📷 "
                + ("No image captured" if self.lang == "en" else "Chưa có hình ảnh"),
                bg="#FAFAFA",
                fg="#9E9E9E",
                font=self.ui_font_label,
            ).pack(expand=True)
    ...
    def _add_skill(self):
        ...
        if dialog.result:
            ...
            messagebox.showinfo(
                "Added" if self.lang == "en" else "Đã Thêm",
                (
                    f"Skill '{dialog.result.get('name', 'Unknown')}' has been added."
                    if self.lang == "en"
        ...
        if not hasattr(self, "current_skill") or self.current_skill is None:
            messagebox.showwarning(
                ...
                (
                    ...
                    if self.lang == "en"
                    else "Vui lòng chọn một kỹ năng trước."
                ),
            )
            return
    ...
    def _delete_skill_image(self):
        ...
        if not messagebox.askyesno(
            ...
            (
                "Delete this skill image?"
                if self.lang == "en"
                else "Xóa hình ảnh kỹ năng này?"
            ),
        ):
            return
        ...
        try:
            ...
            if not self.current_skill["name"]:
                raise ValueError(
                    ...
                    if self.lang == "en"
                    else "Tên không được để trống"
                )
            if not self.current_skill["key"]:
                raise ValueError(
                    "Key cannot be empty"
                    if self.lang == "en"
                    else "Phím không được để trống"
                )
            if self.current_skill["cooldown"] < 0:
                raise ValueError(
                    "Cooldown must be non-negative"
                    if self.lang == "en"
                    else "Thời gian hồi chiêu phải không âm"
                )
            if self.current_skill["cast_time"] < 0:
                raise ValueError(
                    "Cast time must be non-negative"
                    if self.lang == "en"
                    else "Thời gian thi triển phải không âm"
                )

        except ValueError as e:
            messagebox.showerror(
                "Invalid Input" if self.lang == "en" else "Dữ Liệu Không Hợp Lệ", str(e)
            )
            return
        except Exception as e:
    ...
    def _delete_current_skill_inline(self):
        ...
        title = "Confirm Delete" if self.lang == "en" else "Xác Nhận Xóa"
        msg = (
            f"Delete skill '{name}'?" if self.lang == "en" else f"Xóa kỹ năng '{name}'?"
        )
    ...
    def _on_timing_preset_change(self):
        ...
        preset_values = {"slow": 1.0, "normal": 2.0, "fast": 3.0, "very_fast": 4.0}
    ...
    def _calculate_timing(self):
        ...
        if not hasattr(self, "selected_timing_monster"):
            messagebox.showwarning(self._t("error_title"), self._t("timing_no_monster"))
            return

        if not hasattr(self, "selected_timing_skill"):
            messagebox.showwarning(self._t("error_title"), self._t("timing_no_skill"))
            return
        ...
        try:
            ...
            if skill_rotation:
                ...
                result = calculate_timing_from_monster(
                    self.selected_timing_monster, attacks_per_second=aps
                )

            if result is None:
                messagebox.showerror(self._t("error_title"), self._t("timing_no_data"))
                return
            ...
            messagebox.showinfo(
                self._t("success_title"), self._t("timing_calc_success")
            )
    ...
    def _get_timing_confidence(self, result) -> str:
        ...
        if (
            result.monster_hp > 0
            and result.damage_per_hit > 0
            and result.attacks_per_second > 0
        ):
            return "high"
    ...
    def _apply_timing_to_config(self):
        ...
        if (
            not hasattr(self, "timing_calculation_result")
            or self.timing_calculation_result is None
        ):
            messagebox.showwarning(
                self._t("error_title"),
                (
                    "Please calculate timing first"
                    if self.lang == "en"
                    else "Vui lòng tính toán timing trước"
                ),
            )
            return
        ...
        try:
            ...
            messagebox.showerror(
                self._t("error_title"), f"Failed to save hunt config: {str(e)}"
            )
    ...
    def _show_timing_execution_inline(self):
        ...
        self.timing_confirmation_frame.pack(
            fill="both", expand=True, padx=10, pady=(10, 15)
        )
    ...
    def _apply_all_changes(self):
        ...
        try:
            ...
            if not any(changes.values()):
                messagebox.showinfo(
                    ...
                    (
                        "No changes to apply."
                        if self.lang == "en"
                        else "Không có thay đổi để áp dụng."
                    ),
                )
                return
            ...
            messagebox.showerror(
                "Error" if self.lang == "en" else "Lỗi", f"Failed to apply changes: {e}"
            )
    ...
    def _on_window_close(self, force_apply: bool = False):
        ...
        if has_changes and not force_apply:
            ...
            response = messagebox.askyesnocancel(
                self._t("confirm_title"), self._t("changes_pending")
            )
...
class MonsterDialog:
    ...
    def __init__(
        ...
        self.dialog.title(
            "Add Monster"
            if mode == "add" and lang == "en"
            else (
                "Thêm Quái"
                if mode == "add"
                else "Edit Monster" if lang == "en" else "Sửa Quái"
            )
        )
    ...
    def _build_form(self):
        ...
        self.damage_var = tk.StringVar(
            value=str(self.monster.get("damage_per_hit", ""))
        )
        ...
        tk.Label(
            ...
            text=(
                ...
                if self.lang == "en"
                else f"{template_count} template"
            ),
            font=(UIStyle.resolve_font_family("body"), 9),
            fg="gray",
...
class SkillDialog:
    ...
    def __init__(
        ...
        self.dialog.title(
            ...
            if mode == "add" and lang == "en"
                ...
                "Thêm Kỹ Năng"
                if mode == "add"
                else "Edit Skill" if lang == "en" else "Sửa Kỹ Năng"
            )
        )
    ...
    def _build_form(self):
        ...
        cast_time_entry = tk.Entry(
            form_frame,
            textvariable=self.cast_time_var,
            width=40,
            font=(UIStyle.resolve_font_family("body"), 9),
        )
    ...
    def _validate(self) -> bool:
        ...
        if not name:
            messagebox.showerror(
                ...
                (
                    ...
                    if self.lang == "en"
                    else "Vui lòng nhập tên kỹ năng."
                ),
                parent=self.dialog,
            )
            return False
        ...
        if not key:
            messagebox.showerror(
                "Validation Error" if self.lang == "en" else "Lỗi Xác Thực",
                (
                    "Please enter key binding."
                    if self.lang == "en"
                    else "Vui lòng nhập phím tắt."
                ),
                parent=self.dialog,
            )
            return False
        ...
        try:
            ...
            messagebox.showerror(
                "Validation Error" if self.lang == "en" else "Lỗi Xác Thực",
                (
                    "Please enter valid cooldown (non-negative number)."
                    if self.lang == "en"
        ...
        try:
            ...
            messagebox.showerror(
                "Validation Error" if self.lang == "en" else "Lỗi Xác Thực",
                (
                    "Please enter valid cast time (non-negative number)."
                    if self.lang == "en"
