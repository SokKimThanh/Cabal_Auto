...
class OverlaySettingsDialog:
    ...
    def __init__(
        ...
        self.default_config = {
            ...
            "trail": {"enabled": False, "length": 5, "fade": True},
            "stats": {"show_fps": False, "show_count": False, "show_memory": False},
        }
    ...
    def _build_appearance_tab(self, parent: ttk.Frame) -> None:
        ...
        self.alpha_value_label = ttk.Label(
            alpha_frame, text=f"{self.alpha_var.get():.2f}"
        )
        ...
        reset_btn = ttk.Button(
            color_frame, text=self._t("reset_colors"), command=self._reset_colors
        )
    ...
    def _build_performance_tab(self, parent: ttk.Frame) -> None:
        ...
        attach_i18n_tooltip(
            fps_slider, "overlay_fps_tooltip", ns=None, lang_provider=lambda: self.lang
        )
    ...
    def _build_effects_tab(self, parent: ttk.Frame) -> None:
        ...
        self.trail_length_label = ttk.Label(
            trail_slider_frame, text=f"{self.trail_length_var.get()}"
        )
        ...
        ttk.Checkbutton(
            stats_frame, text=self._t("stats_fps"), variable=self.stats_fps_var
        ).pack(anchor=tk.W)

        ttk.Checkbutton(
            stats_frame, text=self._t("stats_count"), variable=self.stats_count_var
        ).pack(anchor=tk.W)
    ...
    def _build_button_bar(self) -> None:
        ...
        # Reset all
        ttk.Button(btn_frame, text=self._t("btn_reset"), command=self._reset_all).pack(
            side=tk.LEFT
        )
        ...
        # Cancel
        ttk.Button(btn_frame, text=self._t("btn_cancel"), command=self._on_cancel).pack(
            side=tk.RIGHT, padx=5
        )

        # Apply
        ttk.Button(
            btn_frame, text=self._t("btn_apply"), command=self._on_apply_clicked
        ).pack(side=tk.RIGHT)
    ...
    def _pick_color(self, state: str) -> None:
        ...
        # Show color picker
        result = colorchooser.askcolor(color=color_hex, title=f"Pick color for {state}")
        ...
        if result[0] is not None:
            ...
            self.color_buttons[f"{state}_label"].config(
...
if __name__ == "__main__":
    ...
    def show_settings():
        dialog = OverlaySettingsDialog(
            parent=root, current_config=test_config, lang="vi", on_apply=on_apply
        )
        dialog.show()

    # Button to open settings
    btn = tk.Button(
        root, text="Open Overlay Settings", command=show_settings, width=20, height=2
    )
    btn.pack(pady=50)
