...
class SetupTab(tk.Frame):
    ...
    def _build_collapsible_group(self, row, title_key, desc_key, content_builder):
        ...
        group_frame.grid(row=row, column=0, columnspan=2, sticky="we", pady=(0, 12))
        ...
        def toggle():
            ...
            if visible:
                ...
                content_frame.grid(row=1, column=0, sticky="we", pady=(4, 0))
            else:
                btn_text_var.set(f"▶ {self._t(title_key)}")
                content_frame.grid_remove()

        btn = ttk.Button(header_frame, textvariable=btn_text_var, command=toggle)
        btn.pack(side="left")

        if desc_key:
            desc_label = tk.Label(header_frame, text=self._t(desc_key), fg="#666", font=("Arial", 8))
    ...
    def _build_hotkeys_content(self, frame):
        ...
        tk.Checkbutton(
            ...
            font=("Arial", 9, "bold"), command=self._on_global_hotkey_toggle,
        ...
        tk.Label(frame, text="Start Hunt:" if self.lang == "en" else "Bắt đầu Hunt:", font=("Arial", 9)).grid(
        ...
        tk.Label(frame, text="Stop Hunt:" if self.lang == "en" else "Dừng Hunt:", font=("Arial", 9)).grid(
    ...
    def _build_advanced_content(self, frame):
        ...
        tk.Entry(frame, textvariable=self.app.setup_press_ms_var, width=8).grid(row=1, column=1, sticky="w", pady=4)
        ...
        tk.Entry(frame, textvariable=self.app.setup_target_cycle_var, width=8).grid(row=1, column=3, sticky="w", pady=4)
        ...
        tk.Entry(frame, textvariable=self.app.setup_search_interval_var, width=8).grid(row=2, column=1, sticky="w", pady=4)
        ...
        tk.Entry(frame, textvariable=self.app.setup_attack_interval_var, width=8).grid(row=2, column=3, sticky="w", pady=4)
        ...
        tk.Entry(frame, textvariable=self.app.setup_lost_timeout_var, width=8).grid(row=3, column=1, sticky="w", pady=4)
        ...
        tk.Entry(frame, textvariable=self.app.setup_attack_duration_var, width=8).grid(row=3, column=3, sticky="w", pady=4)
