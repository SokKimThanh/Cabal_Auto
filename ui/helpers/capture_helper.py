...
def capture_region_and_save(
    ...
    if pyautogui is None:
        message = (
            "pyautogui is required for screenshot."
            if lang == "en"
            else "Cần pyautogui để chụp màn hình."
        )
    ...
    try:
        ...
        msg = (
            "Screenshot failed: {exc}".format(exc=exc)
            if lang == "en"
            else f"Chụp màn hình thất bại: {exc}"
        )
    ...
    if not bbox:
        try:
            messagebox.showinfo(
                "Cancelled",
                "Capture cancelled" if lang == "en" else "Đã hủy chụp",
                parent=parent,
            )
        except Exception:
            pass
    ...
    try:
        ...
        msg = (
            "Crop failed: {exc}".format(exc=exc)
            if lang == "en"
            else f"Cắt ảnh thất bại: {exc}"
        )
        messagebox.showerror("Error", msg, parent=parent)
        return None
    ...
    try:
        ...
        msg = (
            "Save failed: {exc}".format(exc=exc)
            if lang == "en"
            else f"Lưu ảnh thất bại: {exc}"
        )
        messagebox.showerror("Error", msg, parent=parent)
        return None
    ...
    try:
        message = (
            "Screenshot saved: {filename}".format(filename=filename)
            if lang == "en"
            else f"Đã lưu ảnh: {filename}"
        )
