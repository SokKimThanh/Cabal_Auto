...
try:
    ...
    def i18n_t(
        key: str, *, ns: Optional[str] = None, default: Optional[str] = None
    ) -> str:
    ...
    **kwargs,
    ...
    if state == "disabled":
        ...
        if "activebackground" in base_config:
            state_overrides["activebackground"] = _brighten_color(
                base_config["activebackground"], 0.1
            )
    elif state == "danger":
        # Danger: force red colors
        danger_config = get_button_config("red")
        state_overrides["bg"] = danger_config.get("bg", "#C62828")
        state_overrides["activebackground"] = danger_config.get(
            "activebackground", "#B71C1C"
        )
        state_overrides["fg"] = danger_config.get("fg", "white")
        state_overrides["activeforeground"] = danger_config.get(
            "activeforeground", "white"
        )
    ...
    final_config = {**base_config, **state_overrides, **kwargs}
    ...
    if is_photoimage:
        ...
        if text:
            ...
            try:
                ...
                button = tk.Button(parent, image=icon, command=command, **final_config)
            except Exception:
                button = tk.Button(parent, text="?", command=command, **final_config)
    else:
    ...
    if auto_hover_disabled and (
        state == "disabled" or kwargs.get("state") == "disabled"
    ):
        # Get forbidden icon for hover
        forbidden_icon = icon_helper.get_icon(
            "forbidden", fallback="🚫", size=icon_size
        )
    ...
    def _on_focus_out(event):
        try:
            event.widget.config(
                highlightbackground=event.widget.cget("bg"),
                highlightcolor=event.widget.cget("bg"),
...
def create_icon_label(
    ...
    **kwargs,
    ...
    if is_photoimage:
        ...
        if text:
            label = tk.Label(
                parent, image=icon, text=text, compound="left", **label_config
            )
        else:
            # Icon only
            label = tk.Label(parent, image=icon, **label_config)
...
def update_button_state(
    ...
    if enabled:
        ...
        if icon_name:
            icon = icon_helper.get_icon(
                icon_name, fallback=icon_fallback, size=icon_size
            )
        else:
...
def set_button_enabled(
    button: tk.Button, enabled: bool, tooltip: Optional[str] = None
) -> None:
    """
...
def set_button_icon(
    button: tk.Button, icon_name: str, fallback: str = "", size: int = 16
) -> None:
    """
