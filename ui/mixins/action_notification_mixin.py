...
class ActionNotificationMixin(object):
    ...
    def register_action_rules(self, rules: Dict[str, Dict[str, Any]]) -> None:
        ...
        for action_name, action_rules in rules.items():
            if not isinstance(action_rules, dict):
                raise ValueError(
                    f"Rules for '{action_name}' must be a dictionary, got {type(action_rules).__name__}"
                )
            ...
            if unknown_types:
                self._logger.warning(
                    f"Unknown rule types for '{action_name}': {unknown_types}"
                )
            ...
            if "validation" in action_rules:
                ...
                if not isinstance(val_rule, dict):
                    raise ValueError(
                        f"'validation' rule for '{action_name}' must be a dictionary"
                    )
                if "check" not in val_rule or not callable(val_rule["check"]):
                    raise ValueError(
                        f"'validation' rule for '{action_name}' must have a callable 'check'"
                    )
                if "message" not in val_rule or not isinstance(
                    val_rule["message"], str
                ):
                    raise ValueError(
                        f"'validation' rule for '{action_name}' must have a string 'message'"
                    )
                if (
                    "type" in val_rule
                    and val_rule["type"] not in valid_notification_types
                ):
                    raise ValueError(
                        f"Invalid notification type for '{action_name}': {val_rule['type']}"
                    )
            ...
            if "confirmation" in action_rules:
                ...
                if not isinstance(conf_rule, dict):
                    raise ValueError(
                        f"'confirmation' rule for '{action_name}' must be a dictionary"
                    )
                if "check" not in conf_rule or not callable(conf_rule["check"]):
                    raise ValueError(
                        f"'confirmation' rule for '{action_name}' must have a callable 'check'"
                    )
                if "message" not in conf_rule or not isinstance(
                    conf_rule["message"], str
                ):
                    raise ValueError(
                        f"'confirmation' rule for '{action_name}' must have a string 'message'"
                    )
            ...
            for rule_type in ["success", "error"]:
                if rule_type in action_rules:
                    ...
                    if not isinstance(rule, dict):
                        raise ValueError(
                            f"'{rule_type}' rule for '{action_name}' must be a dictionary"
                        )
                    if "message" not in rule or not isinstance(rule["message"], str):
                        raise ValueError(
                            f"'{rule_type}' rule for '{action_name}' must have a string 'message'"
                        )
                    if "type" in rule and rule["type"] not in valid_notification_types:
                        raise ValueError(
                            f"Invalid notification type for '{action_name}': {rule['type']}"
                        )
    ...
    def execute_action(
        self, action_name: str, action_callback: Callable, *args, **kwargs
    ) -> Optional[Any]:
        """
        ...
        if action_name not in self._action_rules:
            raise ValueError(
                f"Action '{action_name}' is not registered. Call register_action_rules() first."
            )

        if self._notification_widget is None:
            raise ValueError(
                "Notification widget not set. Call set_notification_widget() first."
            )
            ...
            try:
                if conf_rule["check"]():
                    ...
                    if not confirmed:
                        if self._debug_mode:
                            self._logger.debug(
                                f"Action '{action_name}' cancelled by user"
                            )
                        return None
            except Exception as e:
    ...
    def _show_notification(
        self, message: str, msg_type: NotificationType = "info"
    ) -> None:
        """
        ...
        if self._notification_widget is None:
            self._logger.warning(
                f"Cannot show notification: widget not set. Message: {message}"
            )
            return
        ...
        if hasattr(self._notification_widget, method_name):
            ...
            self._logger.warning(
                f"Notification widget doesn't have method: {method_name}"
            )
