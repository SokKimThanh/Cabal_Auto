...
4. **Độ phân giải & DPI Scaling (100% – 200%):** Bố cục chuẩn $1920 \times 1080$ (fallback tối thiểu $1280 \times 720$), kích hoạt DPI Awareness, sử dụng `grid()`, `weight`, `minsize` và tính toán tọa độ tương đối.
5. **Nguồn Dữ liệu Thực tế:** Kết hợp hai tầng tra cứu giữa CSDL SQLite `monsters.db` (3948 quái)[cite: 2, 3] và fallback an toàn sang `lib/data/monsters.json`.
...
| **03** | `PROMPT-UX6` | Phase UX | **Collapsible Bottom Logs (Vùng C2/D):** Tạo khung nhật ký $1640 \times 200\text{ px}$ có nút thu gọn/mở rộng `[▲]/[▼]`, Circular Buffer giới hạn 1000 dòng chống tràn RAM, Batch Insert (50 dòng/tick) và ghi file log xoay vòng. | 20–25 phút | `PROMPT-UX2`, `lib/system/hunt_logger.py` |
...
| **07** | `PROMPT-CB2B`| Phase Combat | **Target Name Reader & ID Resolver:** Cắt ROI tên quái trên thanh Target Bar, đọc bằng OCR nhanh và tra cứu CSDL `monsters.db` lấy ID #xxxx và Max HP[cite: 2]. | 25–30 phút | `PROMPT-CB2`, `database.py`[cite: 2] |
| **08** | `PROMPT-UX3` | Phase UX | **Dynamic Monster Rotation Queue (Vùng B - Trái):** Tái cấu trúc Panel trái $776 \times 552\text{ px}$ thành hàng đợi động; phân biệt trực quan quái thật vs quái fallback, hiển thị khoảng cách (rate-limit 5 FPS) và Debounced JSON sync[cite: 2]. | 25–30 phút | `PROMPT-UX2`, `PROMPT-CB2B` |
| **09** | `PROMPT-UX4.1` | Phase UX | **Dual-Lane Skill Strip Layout (Vùng B - Dưới):** Dựng khung dải kỹ năng $1576 \times 120\text{ px}$ chia 2 làn (Combo Chain & Buff Lane), Compact Cards với huy hiệu `⚡ <cast>s` và `⏳ <cd>s`, bộ điều khiển Auto Combo. | 20–25 phút | `PROMPT-UX2` |
...
| **11** | `PROMPT-CB4` | Phase Combat | **Sync Config Schema:** Chuẩn hóa và đồng bộ schema `skill_slots`, `buff_slots`, `monster_rotation` giữa `hunt_config.json`, `config_migrator.py` và `app_gui.py`[cite: 1]. | 20–25 phút | `PROMPT-UX4.2`, `lib/features/hunt/hunt_config.py` |
...
| **14** | `PROMPT-UX5.1` | Phase UX | **Target Card Shell & Fallback Schema (Vùng B - Phải):** Dựng khung thẻ mục tiêu $776 \times 552\text{ px}$, adapter fallback toàn diện cho schema CSDL (tránh `KeyError`), cơ chế giải phóng bộ nhớ Tkinter Image triệt để[cite: 2]. | 20–25 phút | `PROMPT-UX2`, `PROMPT-CB2B` |
...
1. Mở AI Assistant trong môi trường phát triển (Cursor / VS Code).
2. Dán khối quy tắc chung `00-global-rules.md`.
3. Dán tiếp nội dung file `PROMPT-xxx` tương ứng theo thứ tự từ **01 đến 15**.
4. **Phút 00 – 20/25:** Thực hiện viết mã nguồn và unit tests tương ứng.
5. **Phút 20/25 – 30:** Chạy kiểm thử tự động (Pytest / Smoke Test / High-DPI Test).
6. **Nghiệm thu:** Nếu toàn bộ tiêu chí tại `Session Boundary Gate` đạt chuẩn, chuyển tiếp sang phiên tiếp theo.
