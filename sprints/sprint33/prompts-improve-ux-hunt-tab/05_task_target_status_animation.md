...
1. Trong `ui/panels/target_status_panel.py`:
   - Hàm `_on_target_hp_updated`: Không cập nhật tọa độ Canvas HP/MP ngay lập tức. Hãy viết một helper function (hoặc loop nhỏ dùng `.after()`) để nội suy (interpolate) từ vị trí cũ tới vị trí mới trong ~200ms.
...
- **Crashes:** Gọi Canvas coords trong lúc tab đã bị đóng (destroyed) sẽ văng lỗi `_tkinter.TclError`. Cần kiểm tra `self.winfo_exists()` trước khi thay đổi UI trong callback `.after()`.
