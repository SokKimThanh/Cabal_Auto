...
- **Tkinter After Loop:** Không dùng `time.sleep()` trong logic đổi màu. Bắt buộc dùng `self.after()` của Tkinter để thực hiện hiệu ứng mờ (fade) để không block luồng UI chính.
