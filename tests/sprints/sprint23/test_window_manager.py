...
pytestmark = [
    pytest.mark.windows,
    pytest.mark.skipif(
        sys.platform != "win32", reason="Requires Windows OS and pywin32/ctypes"
    ),
]
...
def mock_win32():
    ...
    with patch("lib.system.window_manager.win32gui") as win32gui_mock, patch(
        "lib.system.window_manager.win32con"
    ) as win32con_mock, patch(
        "lib.system.window_manager.win32api"
    ) as win32api_mock, patch(
        "lib.system.window_manager.win32process"
    ) as win32process_mock, patch(
        "lib.system.window_manager.psutil"
    ) as psutil_mock:
        ...
        def get_window_text(hwnd):
            titles = {1001: "Cabal Online", 1002: "Notepad", 1003: "Chrome"}
...
def test_find_window_multiple_criteria(window_manager):
    ...
    hwnd = window_manager.find_window(title_contains="Cabal", process_name="cabal")
