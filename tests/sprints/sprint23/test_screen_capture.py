...
def mock_win32():
    ...
    with patch("lib.system.screen_capture.win32gui") as win32gui_mock, patch(
        "lib.system.screen_capture.win32ui"
    ) as win32ui_mock, patch(
        "lib.system.screen_capture.win32con"
    ) as win32con_mock, patch(
        "lib.system.screen_capture.windll"
    ) as windll_mock:
...
def test_capture_initialization_custom_params():
    ...
    capture = ScreenCapture(queue_size=10, target_fps=30, downsample=(640, 480))
    ...
    assert capture.frame_interval == pytest.approx(
        1.0 / 30
    ), "Frame interval should match target FPS"
...
def test_get_frame_from_queue():
    ...
    assert frame.shape == (
        ...
        100,
        3,
    ), f"Frame shape should be (100, 100, 3), got {frame.shape}"
...
def test_get_stats():
    ...
    assert isinstance(
        stats, CaptureStats
    ), f"Should return CaptureStats, got {type(stats)}"
...
def test_create_capture_success(mock_win32):
    ...
    assert isinstance(
        capture, ScreenCapture
    ), f"Should be ScreenCapture, got {type(capture)}"
...
def test_capture_loop_runs(mock_win32):
    ...
    with patch.object(ScreenCapture, "_capture_frame") as mock_capture_frame:
        ...
        assert (
            not capture.frame_queue.empty()
        ), "Queue should have frames after capture loop runs"
...
def test_resize_during_capture(mock_win32):
    ...
    with patch.object(capture, "_reallocate_buffer") as mock_realloc, patch.object(
        capture, "_capture_frame", return_value=np.zeros((768, 1024, 3), dtype=np.uint8)
    ):
...
def test_concurrent_access_during_reallocation(mock_win32):
    ...
    with patch.object(
        ...
        assert (
            len(exceptions) == 0
        ), f"Concurrent access raised exceptions: {exceptions}"
