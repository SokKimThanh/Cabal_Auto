...
@patch('lib.features.hunt.hunt_orchestrator.get_hunt_logger')
def test_start_hunt_missing_bot_manager(mock_get_logger):
    mock_logger = MagicMock()
    mock_get_logger.return_value = mock_logger

    mock_schedule = MagicMock()
    orchestrator = HuntOrchestrator(
        bot_manager=None, # explicitly pass None
        on_status_update=MagicMock(),
        on_state_change=MagicMock(),
        locate_target=MagicMock(),
        prepare_skill_runtime=MagicMock(),
    ...
    orchestrator.start_hunt({"search_interval": 1.0})
    assert orchestrator.hunt_running is False
    mock_logger.log_error.assert_called_with("dependency_missing", "bot_manager is missing from HuntOrchestrator")
