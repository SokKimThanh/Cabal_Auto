...
def test_skill_stats_class():
    ...
    ), "time_since_last_cast should not be None for casted skill"
    assert (
        time_since >= 0
    ), f"time_since_last_cast should be non-negative, got {time_since}"

    # Test get_success_rate
    fire_ball_rate = stats.get_success_rate("Fire Ball")
    assert (
        fire_ball_rate == 100.0
    ), f"Fire Ball success rate should be 100%, got {fire_ball_rate}%"

    ice_storm_rate = stats.get_success_rate("Ice Storm")
    assert (
        ice_storm_rate == 0.0
    ), f"Ice Storm success rate should be 0%, got {ice_storm_rate}%"

    # Test get_all_stats
    all_stats = stats.get_all_stats()
    assert isinstance(
        all_stats, dict
    ), f"get_all_stats should return dict, got {type(all_stats)}"
    ...
    assert (
        reset_count == 0
    ), f"reset_skill should reset cast count to 0, got {reset_count}"
