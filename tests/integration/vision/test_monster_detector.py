...
class TestMonsterDetectorInit:
    ...
    def test_init_success(self, mock_vision_engine, mock_screen_capture):
        ...
        detector = MonsterDetector(
            vision_engine=mock_vision_engine, screen_capture=mock_screen_capture
        )
...
class TestCallbackSystem:
    ...
    def test_callback_receives_detections(self, detector, mock_vision_engine):
        ...
        mock_detections = [
            Detection(
                x=100, y=100, w=50, h=50, score=0.9, template_id="monster1", scale=1.0
            ),
            Detection(
                x=200, y=200, w=50, h=50, score=0.8, template_id="monster2", scale=1.0
            ),
        ]
...
class TestStateMachine:
    ...
    def test_detected_to_tracking_after_stable_frames(
        self, detector, mock_vision_engine
    ):
    ...
    def test_state_change_triggers_callback(self, detector, mock_vision_engine):
        ...
        assert (
            DetectionState.DETECTED in state_changes
            or DetectionState.TRACKING in state_changes
        )
