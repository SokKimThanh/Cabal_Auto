...
class TestMonsterEditorData:
    ...
    def test_load_monsters_valid_data(
        ...
        temp_data_file.write_text(
            json.dumps(sample_monsters, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    def test_load_monsters_auto_generate_ids(self, patched_monster_editor) -> None:
        ...
        monsters_without_ids = [{"name": "No ID Monster", "level": 1, "templates": []}]

        temp_data_file.write_text(
            json.dumps(monsters_without_ids, indent=2), encoding="utf-8"
        )
    ...
    def test_save_monsters_error_handling(
        self, mock_open, patched_monster_editor
    ) -> None:
