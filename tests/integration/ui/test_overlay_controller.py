...
pytestmark = [
    ...
    pytest.mark.windows,
    pytest.mark.skip(
        reason="Obsolete overlay controller unit test for missing legacy module"
    ),
]
...
def controller(mock_overlay, mock_detector):
    ...
    return OverlayController(overlay=mock_overlay, detector=mock_detector, max_boxes=20)
...
class TestControllerInit:
    ...
    def test_init_success(self, mock_overlay, mock_detector):
        ...
        controller = OverlayController(overlay=mock_overlay, detector=mock_detector)
    ...
    def test_init_with_max_boxes(self, mock_overlay, mock_detector):
        ...
        controller = OverlayController(
            overlay=mock_overlay, detector=mock_detector, max_boxes=10
        )
...
class TestStateHandling:
    ...
    def test_searching_state_clears_overlay(
        self, controller, mock_overlay, mock_detector
    ):
    ...
    def test_state_affects_detection_colors(
        self, controller, mock_overlay, mock_detector
    ):
        ...
        detection = Detection(
            x=100, y=100, w=50, h=50, score=0.9, template_id="m1", scale=1.0
        )
...
class TestErrorHandling:
    ...
    def test_overlay_update_error_continues(
        self, controller, mock_overlay, mock_detector
    ):
    ...
    def test_state_callback_error_continues(
        self, controller, mock_overlay, mock_detector
    ):
...
class TestStatsDisplay:
    ...
    def test_stats_box_formatting(self, mock_overlay, mock_detector):
        ...
        # Find stat boxes
        stat_boxes = [
            box for box in boxes if "FPS:" in box.label or "State:" in box.label
        ]
...
class TestWindowStateHandling:
    ...
    def test_minimize_pauses_detection(self, mock_overlay, mock_detector):
        ...
        controller = OverlayController(
            mock_overlay, mock_detector, window_tracker=mock_tracker
        )
        controller.start()
    ...
    def test_no_tracker_no_errors(self, mock_overlay, mock_detector):
        ...
        controller = OverlayController(
            mock_overlay, mock_detector, window_tracker=None  # No tracker
        )
