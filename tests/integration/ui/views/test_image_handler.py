...
def mock_pil_available():
    with patch("views.image_handler.PIL_AVAILABLE", True), patch(
        "views.image_handler.Image", MagicMock()
    ), patch("views.image_handler.ImageTk", MagicMock()), patch(
        "views.image_handler.ImageDraw", MagicMock()
    ):
        yield
...
def test_get_preview_image_file_not_found(handler, mock_pil_available):
    with patch(
        "views.image_handler.ImageHandler._create_placeholder",
...
def test_get_preview_image_exception_handling(handler, mock_pil_available):
    with patch("views.image_handler.Image.open", side_effect=Exception("Read error")):
        with patch.object(Path, "exists", return_value=True):
            with patch.object(
                handler, "_create_placeholder", return_value="mock_placeholder"
            ):
...
def test_create_placeholder_success(mock_pil_available):
    ...
    with patch("views.image_handler.Image.new", create=True) as mock_new:
        ...
        with patch(
            "views.image_handler.ImageTk.PhotoImage",
            return_value="mock_placeholder_photo",
        ):
