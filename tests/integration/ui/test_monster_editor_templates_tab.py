...
class TestMonsterEditorTemplatesTab:
    ...
    def test_templates_tab_created(self, temp_data_file: Path) -> None:
        ...
        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file):
            ...
            try:
                ...
                assert (
                    editor.templates_tab is not None
                ), "Templates tab should be created"
    ...
    def test_template_listbox_created(self, temp_data_file: Path) -> None:
        ...
        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file):
            ...
            try:
                ...
                assert (
                    editor.template_listbox is not None
                ), "Template listbox should be created"
                assert (
                    editor.template_scrollbar is not None
                ), "Scrollbar should be created"

                # Rule 1: Type check widget types
                assert isinstance(
                    editor.template_listbox, tk.Listbox
                ), "Should be Listbox widget"
                assert isinstance(
                    editor.template_scrollbar, tk.Scrollbar
                ), "Should be Scrollbar widget"

                # Verify listbox configuration
                assert (
                    editor.template_listbox["selectmode"] == tk.SINGLE
    ...
    def test_control_buttons_created(self, temp_data_file: Path) -> None:
        ...
        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file):
            ...
            try:
                ...
                # Rule 2: Check None before access
                assert (
                    editor.capture_button is not None
                ), "Capture button should be created"
                assert (
                    editor.browse_button is not None
                ), "Browse button should be created"
                assert (
                    editor.delete_template_button is not None
                ), "Delete button should be created"
                assert (
                    editor.test_template_button is not None
                ), "Test button should be created"

                # Rule 1: Type check all buttons
                assert isinstance(
                    editor.capture_button, tk.Button
                ), "Should be Button widget"
                assert isinstance(
                    editor.browse_button, tk.Button
                ), "Should be Button widget"
                assert isinstance(
                    editor.delete_template_button, tk.Button
                ), "Should be Button widget"
                assert isinstance(
                    editor.test_template_button, tk.Button
                ), "Should be Button widget"

                # Verify button texts (check translation keys work)
                capture_text = editor.capture_button["text"]
                assert (
                    capture_text is not None and len(capture_text) > 0
                ), "Capture button should have text"

                browse_text = editor.browse_button["text"]
                assert (
                    browse_text is not None and len(browse_text) > 0
                ), "Browse button should have text"

                delete_text = editor.delete_template_button["text"]
                assert (
                    delete_text is not None and len(delete_text) > 0
                ), "Delete button should have text"

                test_text = editor.test_template_button["text"]
                assert (
                    test_text is not None and len(test_text) > 0
    ...
    def test_threshold_slider_created(self, temp_data_file: Path) -> None:
        ...
        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file):
            ...
            try:
                ...
                # Rule 2: Check None before access
                assert (
                    editor.threshold_scale is not None
                ), "Threshold scale should be created"
                assert (
                    editor.threshold_label is not None
                ), "Threshold label should be created"

                # Rule 1: Type check widgets
                assert isinstance(
                    editor.threshold_scale, tk.Scale
                ), "Should be Scale widget"
                assert isinstance(
                    editor.threshold_label, tk.Label
                ), "Should be Label widget"
                ...
                assert (
                    editor.threshold_scale["resolution"] == 0.01
                ), "Should have 0.01 resolution"
                assert (
                    editor.threshold_scale["orient"] == "horizontal"
    ...
    def test_templates_tab_widgets_all_optional_type(
        ...
        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file):
            ...
            try:
                ...
                for widget_name, widget in widgets_to_check:
                    assert (
                        widget is not None
                    ), f"{widget_name} should be created and not None"
