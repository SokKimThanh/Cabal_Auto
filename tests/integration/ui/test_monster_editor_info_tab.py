...
class TestMonsterEditorInfoTab:
    ...
    def test_monster_selection_populates_form(
        self, temp_data_file: Path, sample_monsters: list
    ) -> None:
        """Test that selecting a monster populates the form."""
        temp_data_file.write_text(json.dumps(sample_monsters), encoding="utf-8")

        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file), patch(
            "ui.windows.monster_manager_win.get_db", return_value=None
    ...
    def test_dirty_state_ui_updates(self, temp_data_file: Path) -> None:
        ...
        with patch("ui.windows.monster_manager_win.DATA_PATH", temp_data_file):
            ...
            try:
                ...
                # Initially not dirty
                assert editor.is_dirty is False
                assert editor.status_label.cget("text") in (
                    "All saved",
                    "Đã lưu tất cả",
                )
                ...
                assert (
                    "Unsaved changes" in status_text
                    or "Có thay đổi chưa lưu" in status_text
                )
