...
class TestDetectionBox:
    ...
    def test_to_tkinter_color(self):
        ...
        box = DetectionBox(x=0, y=0, w=10, h=10, label="Test", color=(255, 128, 64))
...
class TestOverlayWindow:
    ...
    def test_create_overlay_window(self, root_window, sample_rect):
        ...
        overlay = OverlayWindow(target_rect=sample_rect, alpha=0.7, fps_limit=15)
    ...
    def test_update_detections_thread_safe(
        self, root_window, sample_rect, sample_detections
    ):
...
class TestOverlayIntegration:
    ...
    @pytest.mark.slow
    def test_overlay_rendering_performance(
        self, root_window, sample_rect, sample_detections
    ):
    ...
    def test_click_through_manual(self, root_window, sample_rect, sample_detections):
        ...
        overlay = OverlayWindow(
            target_rect=sample_rect, alpha=0.7, fps_limit=15, enable_click_through=True
        )
        overlay.create(parent=root_window)
        overlay.show()
        overlay.update_detections(sample_detections)
    ...
    @pytest.mark.manual
    def test_transparency_levels_manual(
        self, root_window, sample_rect, sample_detections
    ):
        ...
        for alpha in [0.3, 0.5, 0.7, 0.9]:
            overlay = OverlayWindow(target_rect=sample_rect, alpha=alpha, fps_limit=15)
            overlay.create(parent=root_window)
            overlay.show()
            overlay.update_detections(sample_detections)
...
class TestOverlayEdgeCases:
    ...
    def test_update_detections_queue_overflow(
        self, root_window, sample_rect, sample_detections
    ):
