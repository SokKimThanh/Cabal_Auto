...
def mock_db_connection():
    ...
    with patch(
        "lib.db.services.schema_migration_service.get_connection",
        return_value=(conn, False),
    ), patch(
        "lib.db.services.seed_classes_service.get_connection",
        return_value=(conn, False),
    ), patch(
...
class TestDB8Integrity:
    def test_schema_and_migration_lifecycle(self, mock_db_connection):
        ...
        columns = [
            col["name"]
            for col in q(mock_db_connection, "PRAGMA table_info(synergy_effects)")
        ]
    ...
    def test_idempotency_and_parity_checks(self, mock_db_connection):
        ...
        counts_run_1 = {
            ...
            "synergy_effects": len(
                q(mock_db_connection, "SELECT * FROM synergy_effects")
            ),
            "class_skill_assignments": len(
                q(mock_db_connection, "SELECT * FROM class_skill_assignments")
            ),
        }
    ...
    def test_constraint_and_relationship_integrity(self, mock_db_connection):
        ...
        duplicates_classes = q(
            mock_db_connection,
            "SELECT class_code, COUNT(*) FROM classes WHERE class_code IS NOT NULL GROUP BY class_code HAVING COUNT(*) > 1",
        )
        assert len(duplicates_classes) == 0

        duplicates_skills = q(
            mock_db_connection,
            "SELECT skill_code, COUNT(*) FROM skills WHERE skill_code IS NOT NULL GROUP BY skill_code HAVING COUNT(*) > 1",
        )
        assert len(duplicates_skills) == 0

        duplicates_assignments = q(
            mock_db_connection,
            "SELECT class_id, skill_id, COUNT(*) FROM class_skill_assignments GROUP BY class_id, skill_id HAVING COUNT(*) > 1",
        )
        ...
        class_with_assignments = q(
            mock_db_connection, "SELECT class_id FROM class_skill_assignments LIMIT 1"
        )[0]["class_id"]
        mock_db_connection.execute("PRAGMA foreign_keys = ON")
        try:
            mock_db_connection.execute(
                "DELETE FROM synergies WHERE class_id = ?", (class_with_assignments,)
            )
        except:
            pass
        mock_db_connection.execute(
            "DELETE FROM classes WHERE class_id = ?", (class_with_assignments,)
        )
        mock_db_connection.commit()
        assignments_after = q(
            mock_db_connection,
            "SELECT * FROM class_skill_assignments WHERE class_id = ?",
            (class_with_assignments,),
        )
        ...
        skill_with_assignments = q(
            mock_db_connection, "SELECT skill_id FROM class_skill_assignments LIMIT 1"
        )[0]["skill_id"]
        mock_db_connection.execute("PRAGMA foreign_keys = ON")
        mock_db_connection.execute(
            "DELETE FROM skills WHERE skill_id = ?", (skill_with_assignments,)
        )
        mock_db_connection.commit()
        assignments_after_skill = q(
            mock_db_connection,
            "SELECT * FROM class_skill_assignments WHERE skill_id = ?",
            (skill_with_assignments,),
        )
        ...
        missing_skill_code = q(
            mock_db_connection, "SELECT COUNT(*) FROM skills WHERE skill_code IS NULL"
        )[0][0]
    ...
    def test_runtime_adapter_protection(self, mock_db_connection):
        ...
        from lib.db.adapters.catalogue_adapters import (
            MonsterCatalogueLookup,
            SkillCatalogueLookup,
        )
