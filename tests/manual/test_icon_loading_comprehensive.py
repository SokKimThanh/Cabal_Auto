...
def test_icon_loading():
    ...
    try:
        ...
        print(
            f"    PIL Version: {Image.__version__ if hasattr(Image, '__version__') else 'Unknown'}"
        )
    ...
    for icon_name in test_icons:
        ...
        if is_emoji:
            ...
            print(
                f"  [IMAGE] {icon_name:10} -> PhotoImage (type: {type(result).__name__})"
            )
    ...
    for icon_name in test_icons:
        ...
        if is_emoji:
            ...
            # Icon button
            btn = tk.Button(frame, image=icon, bg="#4CAF50", width=30, height=30)
    ...
    tk.Label(test_frame, text="vs", font=("Arial", 10), bg="#f0f0f0").pack(
        side="left", padx=10
    )
