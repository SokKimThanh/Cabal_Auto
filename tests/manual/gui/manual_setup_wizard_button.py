...
# Mark as GUI test (requires display)
...
def test_open_wizard():
    ...
    try:
        status_label.config(
            text="Status: Opening Setup Wizard...", bg="#17a2b8", fg="white"
        )
        root.update()
        ...
        def on_cancel():
            status_label.config(
                text="Status: Wizard cancelled", bg="#6c757d", fg="white"
            )
            print("ℹ️ Wizard cancelled")

        # Call the function
        show_setup_wizard(
            root, config_manager=None, on_complete=on_complete, on_cancel=on_cancel
        )

        status_label.config(
            text="Status: Wizard opened successfully!", bg="#28a745", fg="white"
        )
        print("✅ Setup Wizard opened successfully!")

    except Exception as e:
        status_label.config(
            text=f"Status: Error - {str(e)[:50]}", bg="#dc3545", fg="white"
        )
