...
def demo_app_and_game():
    ...
    log_frame = tk.LabelFrame(
        root, text="Event Log", font=("Segoe UI", 9), bg="#F5F5F5", padx=10, pady=5
    )
    log_frame.pack(pady=10, padx=20, fill="both", expand=True)

    log_text = tk.Text(log_frame, height=8, font=("Consolas", 9), bg="#FFFFFF")
...
def demo_custom_modes():
    ...
    title = tk.Label(
        root, text="Custom Window Selector", font=("Segoe UI", 14, "bold"), bg="#F5F5F5"
    )
    title.pack(pady=20)

    # Status
    status = tk.Label(
        root, text="Mode: normal", font=("Segoe UI", 11), bg="#F5F5F5", fg="#FF5722"
    )
    status.pack(pady=10)
...
def demo_multiple_windows():
    ...
    sel1 = create_app_window_selector(
        parent=frame1, config_path="tests/tmp/main_app_config.json"
    )
    sel1.pack()
    ...
    sel2 = create_game_window_selector(
        parent=frame2, config_path="tests/tmp/game_window_config.json"
    )
    sel2.pack()
