...
def demo_basic():
    ...
    # Title
    title = tk.Label(
        root, text="Demo 1: Basic Selector", font=("Segoe UI", 14, "bold"), bg="#F5F5F5"
    )
    title.pack(pady=20)
    ...
    selector = create_game_window_mode_selector(
        parent=frame, config_path="tests/tmp/demo_config.json"
    )
    selector.pack()
...
def demo_with_callback():
    ...
    log_text = tk.Text(
        log_frame, height=8, width=50, font=("Consolas", 9), bg="#FFFFFF", fg="#333333"
    )
...
def demo_programmatic_control():
    ...
    # Create selector
    selector = create_game_window_mode_selector(
        parent=selector_frame, config_path="tests/tmp/demo_config.json", show_label=True
    )
    selector.pack()
    ...
    tk.Button(button_frame, text="Set: None", command=set_none, width=12).pack(
        side="left", padx=5
    )

    tk.Button(button_frame, text="Set: Below", command=set_below, width=12).pack(
        side="left", padx=5
    )

    tk.Button(button_frame, text="Set: Above", command=set_above, width=12).pack(
        side="left", padx=5
    )

    tk.Button(button_frame, text="Get Current", command=get_current, width=12).pack(
        side="left", padx=5
    )
