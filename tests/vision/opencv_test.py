...
class TemplateMatchTester:
    ...
    def test_opencv_match(
        ...
        try:
            ...
            screenshot_gray = cv2_module.cvtColor(
                screenshot_np, cv2_module.COLOR_RGB2GRAY
            )

            # Match template
            result = cv2_module.matchTemplate(
                screenshot_gray, template_gray, cv2_module.TM_CCOEFF_NORMED
            )
        ...
        try:
            ...
            try:
                box = pyautogui_module.locateOnScreen(
                    template_path, confidence=confidence, region=region, grayscale=True
                )
            except TypeError:
                # Fallback to basic matching without confidence
                box = pyautogui_module.locateOnScreen(
                    template_path, region=region, grayscale=True
                )
    ...
    def compare_methods(
        ...
        if HAS_OPENCV:
            ...
            for i in range(iterations):
                ...
                if "error" not in result:
                    ...
                    print(
                        f"  Iteration {i+1}: {'✅ Found' if result['found'] else '❌ Not found'} "
        ...
        if opencv_times:
            ...
            print(
                f"  • Detection rate: {opencv_found}/{iterations} ({opencv_rate:.1f}%)"
            )
        ...
        if pyautogui_times:
            ...
            print(f"\n📊 PyAutoGUI (locateOnScreen):")
            print(
                f"  • Detection rate: {pyautogui_found}/{iterations} ({pyautogui_rate:.1f}%)"
            )
        ...
        if opencv_times and pyautogui_times:
            ...
            if opencv_avg < pyautogui_avg:
                ...
                print(
                    f"⚡ OpenCV: {speedup:.1f}x faster ({opencv_avg:.2f}ms vs {pyautogui_avg:.2f}ms)"
                )
            else:
                speedup = opencv_avg / pyautogui_avg
                print(
                    f"⚡ PyAutoGUI: {speedup:.1f}x faster ({pyautogui_avg:.2f}ms vs {opencv_avg:.2f}ms)"
                )

            if opencv_confidences:
                print(
                    f"📈 OpenCV provides actual confidence values (avg: {sum(opencv_confidences)/len(opencv_confidences):.3f})"
                )
        ...
        return {
            "opencv": {
                ...
                "avg_confidence": (
                    sum(opencv_confidences) / len(opencv_confidences)
                    if opencv_confidences
                    else 0
                ),
            },
