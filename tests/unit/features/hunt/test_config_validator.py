...
def test_validate_hunt_area_list_bounds():
    hunt_area = {"window_title": "Cabal", "window_bounds": [10, 20, 800, 600]}
    assert validate_hunt_area(hunt_area) == {
        "window_title": "Cabal",
...
def test_validate_hunt_area_malformed_title():
    hunt_area = {"window_title": 123, "window_bounds": [10, 20, 800, 600]}
    assert validate_hunt_area(hunt_area) == {
        "window_title": "123",
        "window_bounds": [10, 20, 800, 600],
    }
...
def test_validate_hunt_area_malformed_bounds():
    hunt_area = {"window_title": "Game", "window_bounds": "invalid"}
    assert validate_hunt_area(hunt_area) == {
        "window_title": "Game",
        "window_bounds": None,
    }
...
def test_normalize_window_bounds_value_minimized():
    ...
    assert (
        normalize_window_bounds_value(
            {"left": -32000, "top": -32000, "width": 100, "height": 100}
        )
        is None
    )
...
def test_get_valid_hunt_area_preserves_keys():
    ...
    assert result == {"window_title": "MyWindow", "window_bounds": [5, 10, 640, 480]}
