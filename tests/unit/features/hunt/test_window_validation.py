...
def test_validate_valid_window(monkeypatch, valid_info):
    monkeypatch.setattr(
        "lib.features.hunt.window_selection_service.WindowManager",
        lambda: MockWindowManager(info=valid_info),
    )
...
def test_validate_window_closed(monkeypatch, valid_info):
    monkeypatch.setattr(
        "lib.features.hunt.window_selection_service.WindowManager",
        lambda: MockWindowManager(info=valid_info, valid=False),
    )
    selected = {"hwnd": 123, "pid": 456}
    res = validate_selected_cabal_window(selected, [{"hwnd": 123}])
    assert not res.is_valid
