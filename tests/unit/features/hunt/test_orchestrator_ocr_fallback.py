...
def test_ocr_fallback_contract():
    class MockHandler(HuntStatusHandler):
        ...
        def update_skill_stats_display(self, stats):
            pass

        def bring_window_to_front(self, name):
            return True

        def bring_window_to_front_by_hwnd(self, hwnd):
            return True

        def bring_window_to_front_by_pid(self, pid):
            return True

        def iconify_app(self):
            pass

        def locate_target(self, params):
            return (0, 0)

        def prepare_skill_runtime(self, skill_def):
            return []

        def try_cast_skills(self):
            pass

        def on_scene_monsters_detected(self, monsters):
            pass
        ...
        with patch("lib.features.hunt.hunt_orchestrator.TargetBarDetector") as MockTBD:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "lib.features.hunt.window_selection_service.validate_selected_cabal_window"
                    ) as mock_val:
                        ...
                        assert (
                            ...
                            in mock_handler.set_target_info_args[-1]
                        )
