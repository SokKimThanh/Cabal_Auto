...
def test_calculator_logic():
    ...
    result1 = calculate_timing(
        monster_hp=10000, damage_per_hit=175, attacks_per_second=2.0
    )
    print(result1)
    ...
    assert (
        result1.estimated_kill_time_sec == 29.0
    ), f"Expected 29s kill time, got {result1.estimated_kill_time_sec}"
    print("✅ Test 1 passed!")
    ...
    result3 = calculate_timing(
        monster_hp=1000, damage_per_hit=500, attacks_per_second=2.0
    )
    print(result3)
    ...
    print("\n\n📊 Test 5: Calculate from Monster Dict")
    print("-" * 70)
    monster = {"name": "Test Dragon", "hp": 50000, "damage_per_hit": 750}
    ...
    print("-" * 70)
    invalid_monster = {"name": "Invalid", "hp": 0, "damage_per_hit": 100}  # Invalid HP
...
def test_calculation_accuracy():
    ...
    for scenario in scenarios:
        ...
        # Verify
        assert (
            result.hits_to_kill == scenario["expected_hits"]
        ), f"Expected {scenario['expected_hits']} hits, got {result.hits_to_kill}"
        assert (
            result.estimated_kill_time_sec == scenario["expected_time"]
