...
def test_icon_availability():
    ...
    print(f"\n📁 Icon Files:")
    print(
        f"   save.ico: {'✅ EXISTS' if ico_path.exists() else '❌ NOT FOUND'} ({ico_path})"
    )
    if ico_path.exists():
    ...
    print(
        f"   save.png: {'✅ EXISTS' if png_path.exists() else '❌ NOT FOUND'} ({png_path})"
    )
    if png_path.exists():
    ...
    assert (
        ico_path.exists() or png_path.exists()
    ), "At least one icon format (ico/png) should exist"
...
def test_monster_dialog_icon():
    ...
    dialog_en = MonsterDialog(
        root, lang="en", mode="add", icon_helper=icon_helper, i18n_registry=i18n_t
    )
    ...
    dialog_vi = MonsterDialog(
        root, lang="vi", mode="add", icon_helper=icon_helper, i18n_registry=i18n_t
    )
