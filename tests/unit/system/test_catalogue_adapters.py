...
def mock_db_connection():
    ...
    cursor.execute(
        "INSERT INTO classes (class_id, name, class_code, icon_path) VALUES (1, 'Warrior', 'warrior', 'icon.png')"
    )
    cursor.execute(
        "INSERT INTO skills (skill_id, name, alias, icon_x, icon_y, icon_w, icon_h, class_id, type) VALUES (1, 'Slash', 'slash_alias', 10, 20, 30, 40, 1, 'attack')"
    )
    cursor.execute(
        "INSERT INTO skills (skill_id, name, alias, icon_x, icon_y, icon_w, icon_h, class_id, type) VALUES (2, 'Unmapped Skill', 'unmapped', 0, 0, 0, 0, NULL, 'utility')"
    )

    conn.commit()

    with patch(
        "lib.db.adapters.catalogue_adapters.get_connection", return_value=(conn, False)
    ) as mock_get_conn:
        yield conn
...
def test_skill_runtime_view_without_catalogue():
    user_skill = {"name": "My Custom Slash", "key": "5", "cooldown": 1.2}
