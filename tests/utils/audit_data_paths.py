...
def analyze_path_line(filepath: str, line_num: int, line: str):
    ...
    patterns = [
        ...
        (
            r"Path\(__file__\)\.parent\.parent\s*/\s*'data'",
            "CHECK: Depends on file location",
        ),
...
for filepath in files_to_check:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if "data" in line.lower() and (
            ".json" in line or "Path(" in line or "data/" in line
        ):
            analysis = analyze_path_line(filepath, i, line.strip())
            if analysis:
                found_refs.append(
                    {"line": i, "code": line.strip(), "analysis": analysis}
                )
    ...
    if found_refs:
        for ref in found_refs:
            status_marker = (
                ...
                else "❌" if "WRONG" in ref["analysis"] else "⚠️"
            )
