"""
Simple i18n registry for the whole app.
...
- t(key, *, ns=None, lang=None, default=None): Retrieve a string by key. If ns is provided, lookup under that namespace; otherwise search global fallback and all namespaces.
- get_lang(): Get the current language.

Design goals:
- Non-invasive: existing modules can keep local dictionaries and call register() to expose them.
- Safe: returns the key itself if not found.
- Lightweight: in-memory dicts, no dependencies.
"""
