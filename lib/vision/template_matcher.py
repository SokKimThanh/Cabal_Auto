...
def locate_template_pyautogui(
    ...
    try:
        ...
        box = pyautogui.locateOnScreen(
            template_path, confidence=threshold, region=region, grayscale=True
        )
    except Exception:
        # Fallback to basic matching without confidence (works without OpenCV)
        try:
            box = pyautogui.locateOnScreen(template_path, region=region, grayscale=True)
        except Exception:
            box = None
...
def get_available_methods() -> dict:
    ...
    return {
        "opencv": {
            ...
            "features": (
                ["accurate_confidence", "fast", "grayscale_matching"]
                if HAS_OPENCV
                else []
            ),
        },
        ...
        "recommended": (
            "opencv" if HAS_OPENCV else "pyautogui" if HAS_PYAUTOGUI else None
        ),
    }
