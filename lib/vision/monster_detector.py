...
class MonsterDetector:
    ...
    def start(self, detection_interval: float = 0.1) -> bool:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._detection_loop, name="MonsterDetector", daemon=True
            )
            self._thread.start()
    ...
    def on_detections_changed(
        self, callback: Callable[[List[Detection]], None]
    ) -> None:
        """
    ...
    def _trigger_detection_callbacks(self, detections: List[Detection]) -> None:
        ...
        for callback in callbacks:
            try:
                ...
                logger.error(f"[MonsterDetector] Callback error: {e}", exc_info=True)
    ...
    def _trigger_state_callbacks(self, new_state: DetectionState) -> None:
        ...
        for callback in callbacks:
            try:
                ...
                logger.error(
                    f"[MonsterDetector] State callback error: {e}", exc_info=True
                )
    ...
    def _update_state(self, detections: List[Detection]) -> None:
        ...
        if current_state == DetectionState.SEARCHING:
            if has_detections:
                ...
                logger.info(
                    f"[MonsterDetector] State: SEARCHING -> DETECTED ({len(detections)} found)"
                )
