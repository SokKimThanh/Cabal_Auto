...
class DataSyncManager:
        ...
        try:
            ...
            if "training_monster_list" in config:
                ...
                config["training_monster_list"] = [
                    mid for mid in config["training_monster_list"] if mid != monster_id
                ]
                removed = original_count - len(config["training_monster_list"])
                if removed > 0:
                    print(
                        f"[DataSyncManager] Removed {removed} ID from training_monster_list"
                    )
                    changes_made = True
    ...
    def sync_monster_update(self, monster: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(
                f"[DataSyncManager] Syncing update for monster: {monster.get('name', 'Unknown')}"
            )
        ...
        try:
            ...
            if "monster_rotation" in config and isinstance(
                ...
                config["monster_rotation"] = [
                    ...
                    if isinstance(m, dict) and m.get("monster_id") in valid_ids
                ]
                print(
                    f"[DataSyncManager] Monster list: {len(original_list)} -> {len(config['monster_rotation'])} IDs"
                )
            ...
            if "training_monster_list" in config:
                ...
                config["training_monster_list"] = [
                    mid for mid in original_list if mid in valid_ids
                ]
                print(
                    f"[DataSyncManager] Training list: {len(original_list)} -> {len(config['training_monster_list'])} IDs"
                )
    ...
    def validate_data_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            hunt_ids = [
                m.get("monster_id")
                for m in config.get("monster_rotation", [])
    ...
    def fix_orphaned_references(self) -> bool:
        ...
        try:
            ...
            if "monster_rotation" in config and isinstance(
                ...
                if removed > 0:
                    print(
                        f"[DataSyncManager] Removed {removed} orphaned IDs from monster_rotation"
                    )
            ...
            if "training_monster_list" in config:
                original_count = len(config["training_monster_list"])
                config["training_monster_list"] = [
                    mid for mid in config["training_monster_list"] if mid in valid_ids
                ]
                removed = original_count - len(config["training_monster_list"])
                if removed > 0:
                    print(
                        f"[DataSyncManager] Removed {removed} orphaned IDs from training_monster_list"
                    )
