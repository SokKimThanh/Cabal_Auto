...
class CabalComboDetector:
    ...
    def wait_for_hit_zone(
            ...
            if self._check_hit_zone(frame):
                ...
                while time.time() < guard_end_time:
                    ...
                    if is_target_alive_check and not is_target_alive_check():
                        logger.debug(
                            "Target died during cooldown guard - exiting early"
                        )
                        return True
        ...
        # Timeout reached
        logger.debug(
            f"Combo timeout reached ({timeout_sec}s) without hit-zone detection"
        )
        return False
    ...
    def _check_hit_zone(self, frame: np.ndarray) -> bool:
        ...
        try:
            ...
            if np.any(hit_zone_pixels > self.HSV_VALUE_THRESHOLD):
                logger.debug(
                    f"Bright pixel detected at hit-zone: max_v={np.max(hit_zone_pixels)}"
                )
                return True
