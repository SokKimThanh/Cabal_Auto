...
def calculate_timing(
    ...
    if skill_rotation and len(skill_rotation) > 0:
        ...
        if not attack_skills:
            ...
            avg_cast_time = sum(s.get("cast_time", 1.0) for s in attack_skills) / len(
                attack_skills
            )
            avg_cooldown = sum(s.get("cooldown", 1.5) for s in attack_skills) / len(
                attack_skills
            )
            ...
            if rotation_cycle_time < max_cooldown:
                longest_skills = [
                    ...
                    if s.get("cooldown", 1.5) == max_cooldown
                ]
                logger.warning(
    ...
    attack_duration = max(
        min_attack_duration, min(max_attack_duration, attack_duration)
    )
    ...
    if (
        skill_rotation
        and len([s for s in skill_rotation if s.get("type") == "attack"]) > 0
    ):
        attack_skills = [s for s in skill_rotation if s.get("type") == "attack"]
        avg_cast_time_ms = (
            sum(s.get("cast_time", 1.0) for s in attack_skills)
            / len(attack_skills)
            * 1000
        )
...
def format_timing_recommendation(
    rec: TimingRecommendation, language: str = "en"
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    rec1 = calculate_timing(
        monster_hp=10000, damage_per_hit=500, attacks_per_second=2.0
    )
    print(rec1)
    ...
    rec2 = calculate_timing(
        monster_hp=100000, damage_per_hit=1000, attacks_per_second=2.0
    )
    print(rec2)
    ...
    print("-" * 70)
    rec3 = calculate_timing(monster_hp=1000, damage_per_hit=500, attacks_per_second=2.0)
    print(rec3)
    ...
    rec4 = calculate_timing(
        monster_hp=10000, damage_per_hit=300, attacks_per_second=4.0
    )
    print(rec4)
