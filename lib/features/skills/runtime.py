...
class SkillRuntime:
    ...
    def reserve_next_skill(
        self, lane: str, current_time: float, is_combo: bool = False
    ) -> Optional[CastReservation]:
        """
        ...
        if lane == "attack":
            ...
            if skill.is_ready(current_time):
                ...
                res = CastReservation(
                    ...
                    expected_strategy="combo" if is_combo else "standard",
                )
    ...
    def commit_cast(
        self, token: str, outcome: CastOutcome, current_time: float
    ) -> None:
            ...
            if outcome == CastOutcome.ACCEPTED:
                ...
                if res.lane == "attack":
                    if res.expected_strategy == "combo":
                        self.combo_rotation_index = (
                            self.combo_rotation_index + 1
                        ) % len(self.attack_skills)
                    else:
                        self.attack_rotation_index = (
                            self.attack_rotation_index + 1
                        ) % len(self.attack_skills)
    ...
    def get_status(self, current_time: float) -> Dict:
        ...
        """
        status = {"attacks": [], "buffs": []}

        for skill in self.attack_skills:
            time_until_ready = max(
                0, skill.cooldown - (current_time - skill.last_cast_time)
            )
        ...
        for skill in self.buff_skills:
            time_since_cast = (
                current_time - skill.last_cast_time if skill.last_cast_time > 0 else 0
            )
            time_until_expire = (
                max(0, skill.duration_sec - time_since_cast)
                if skill.duration_sec > 0
                else 0
            )
            ...
            status["buffs"].append(
                {
                    ...
                    "active": (
                        time_since_cast < skill.duration_sec
                        if skill.duration_sec > 0
                        else False
                    ),
                }
...
if __name__ == "__main__":
    ...
    if skills_path.exists():
        ...
        for skill in runtime.buff_skills:
            duration_info = (
                f", duration: {skill.duration_sec}s" if skill.duration_sec > 0 else ""
            )
            refresh_info = (
                f", refresh: {skill.pre_refresh_sec}s"
                if skill.pre_refresh_sec > 0
                else ""
            )
            print(
                f"   • {skill.name} (key: {skill.key}, cooldown: {skill.cooldown}s{duration_info}{refresh_info})"
            )
        ...
        if status["buffs"]:
            ...
            for s in status["buffs"]:
                ...
                expire = (
                    f"{s['time_until_expire']:.1f}s"
                    if s["time_until_expire"] > 0
                    else "expired"
                )
                print(
                    f"      {ready_icon}{active_icon}{refresh_icon} {s['name']} (key {s['key']}): {expire}"
                )
