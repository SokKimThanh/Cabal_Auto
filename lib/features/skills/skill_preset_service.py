...
class SkillPresetService:
    ...
    def create_custom_preset(
        self, class_id: int, name: str, skill_slots: Dict[str, List[int]]
    ) -> Dict[str, Any]:
    ...
    def update_custom_preset(
        self, preset_id: int, skill_slots: Dict[str, List[int]]
    ) -> bool:
    ...
    def migrate_legacy_presets(
        self, legacy_cfg: dict, class_id: int, class_name: str
    ) -> None:
        ...
        if not has_default:
            preset_id = self.preset_repo.create_preset(
                class_id, "Migrated Legacy Preset", is_default=1
            )
