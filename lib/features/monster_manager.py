...
class MonsterManager:
            ...
            if os.path.exists(self.data_path):
                with open(self.data_path, "r", encoding="utf-8") as f:
                    ...
                    if isinstance(data, dict):
                        ...
                        self.monsters = {}
                logger.info(
                    f"Successfully loaded {len(self.monsters)} monsters from {self.data_path}"
                )
            else:
                logger.info(
                    f"Monster data file not found at {self.data_path}. Starting with empty database."
                )
                self.monsters = {}
    ...
    def add_template(
        self, monster_id: str, template_path: str, threshold: float = 0.7
    ) -> bool:
        """
