...
class SkillRotationUI:
    ...
    def _build_ui(self):
        ...
        main_scrollbar = ttk.Scrollbar(
            self.parent, orient="vertical", command=main_canvas.yview
        )
        ...
        title_label = tk.Label(
            ...
            text=(
                ...
                if self.lang == "en"
                else "🎮 XÂY DỰNG CHU KỲ CHIÊU THỨC"
            ),
            font=("Arial", 16, "bold"),
            bg="#1976D2",
            fg="white",
    ...
    def _build_available_skills_panel(self, parent):
        ...
        header = tk.LabelFrame(
            left_frame,
            text="📚 "
            + ("Available Skills" if self.lang == "en" else "Các Chiêu Có Sẵn"),
            font=("Arial", 11, "bold"),
            bg="#E8F5E9",
            fg="#2E7D32",
            padx=10,
        ...
        list_scrollbar = ttk.Scrollbar(
            header, orient="vertical", command=list_canvas.yview
        )
    ...
    def _build_rotation_sequence_panel(self, parent):
        ...
        seq_scrollbar = ttk.Scrollbar(
            header, orient="vertical", command=seq_canvas.yview
        )
    ...
    def _build_analysis_panel(self, parent):
        ...
        analysis_frame = tk.LabelFrame(
            parent,
            text="📊 "
            + ("Rotation Analysis" if self.lang == "en" else "Phân Tích Chu Kỳ"),
            font=("Arial", 11, "bold"),
            bg="#FFF3E0",
            fg="#E65100",
            padx=10,
        ...
        self._update_analysis_text(
            "ℹ️  "
            + (
                "Add skills to rotation and click 'Calculate' to see analysis"
                if self.lang == "en"
    ...
    def _build_action_buttons(self, parent):
        ...
        self.calculate_btn = tk.Button(
            btn_frame,
            text="⚡ "
            + ("Calculate Rotation" if self.lang == "en" else "Tính Toán Chu Kỳ"),
        ...
        self.apply_btn = tk.Button(
            ...
            text="✅ "
            + (
                "Apply to Hunt Config"
                if self.lang == "en"
                else "Áp Dụng Vào Hunt Config"
            ),
        ...
        self.save_preset_btn = tk.Button(
            btn_frame,
            text="💾 "
            + ("Save as Preset" if self.lang == "en" else "Lưu Thành Preset"),
    ...
    def _render_available_skills(self):
        ...
        for i, skill in enumerate(self.available_skills):
            ...
            cb = tk.Checkbutton(
                skill_frame, variable=var, bg=skill_frame["bg"], font=("Arial", 10)
            )
            cb.pack(side="left")
    ...
    def _move_skill(self, index: int, direction: int):
        ...
        if 0 <= new_index < len(self.rotation_sequence):
            ...
            self.rotation_sequence[index], self.rotation_sequence[new_index] = (
                self.rotation_sequence[new_index],
                self.rotation_sequence[index],
            )
            self._render_rotation_sequence()
    ...
    def _clear_rotation(self):
        ...
        if self.rotation_sequence:
            confirm = messagebox.askyesno(
                ...
                (
                    "Clear all skills from rotation?"
                    if self.lang == "en"
    ...
    def _calculate_rotation(self):
        ...
        if not self.rotation_sequence:
            messagebox.showwarning(
                "Warning" if self.lang == "en" else "Cảnh Báo",
                (
                    "Please add skills to rotation first"
                    if self.lang == "en"
        ...
        try:
            ...
            messagebox.showinfo(
                ...
                (
                    ...
                    if self.lang == "en"
                    else f"Đã tính toán chu kỳ thành công!\nThời gian: {self.current_rotation.total_cycle_time:.2f}s"
                ),
            )

        except Exception as e:
            messagebox.showerror(
                "Error" if self.lang == "en" else "Lỗi", f"Calculation failed: {str(e)}"
            )
    ...
    def _apply_rotation(self):
        ...
        if not self.current_rotation:
            messagebox.showwarning(
                "Warning" if self.lang == "en" else "Cảnh Báo",
                (
                    "Please calculate rotation first"
                    if self.lang == "en"
        ...
        try:
            ...
            messagebox.showinfo(
                "Success" if self.lang == "en" else "Thành Công",
                (
                    "Rotation applied to hunt_config.json successfully!"
                    if self.lang == "en"
    ...
    def _save_preset(self):
        ...
        messagebox.showinfo(
            ...
            (
                "Preset saving feature coming soon!"
                if self.lang == "en"
