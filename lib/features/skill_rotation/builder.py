...
def calculate_rotation_timing(skills: List[dict]) -> SkillRotation:
    ...
    for i in range(len(skill_timings) - 1):
        ...
        current_skill.wait_after_cast = (
            next_skill.start_time - current_skill.cast_finish_time
        )
    ...
    if attack_skills:
        ...
        avg_press_ms = sum(s.press_duration_ms for s in attack_skills) // len(
            attack_skills
        )
    else:
        avg_attack_interval = total_cycle_time / len(skill_timings)
        avg_press_ms = sum(s.press_duration_ms for s in skill_timings) // len(
            skill_timings
        )
