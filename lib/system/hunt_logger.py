...
class HuntLogger:
    ...
    def __init__(self, log_dir="logs", max_bytes=10 * 1024 * 1024, backup_count=5):
        ...
        self.logger.info("=" * 80)
        self.logger.info(
            f'Hunt Session Started: {self.session_start.strftime("%Y-%m-%d %H:%M:%S")}'
        )
        self.logger.info("=" * 80)

    def log_match(
        self, template_name, box, threshold=None, confidence=None, monster_name=None
    ):
        """
    ...
    def log_hunt_start(self, config):
        ...
        self.logger.info(
            f"Timing: search={config.get('search_interval')}s, attack={config.get('attack_interval')}s"
        )
        self._log_structured(info)
    ...
    def log_hunt_stop(self, reason="manual"):
        ...
        info = {"event": "HUNT_STOP", "reason": reason, "duration_sec": duration}

        self.logger.info(
            f"[STOP] Hunt stopped | Reason: {reason} | Duration: {duration:.1f}s"
        )
        self.logger.info("=" * 80)
        self._log_structured(info)
