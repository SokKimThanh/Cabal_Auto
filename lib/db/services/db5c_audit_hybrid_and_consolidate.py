...
def run_audit(content=None):
    ...
    try:
        ...
        print(
            f"Error: Could not find source file at {SOURCE_FILE}. Simulating empty/malformed source."
        )
        content = ""
            ...
            rej_list.append(
                {
                    ...
                    "reason": f"Skill '{snake_case_slug}' not found in skill_sprite_catalogue and has no valid alias",
                }
            )
    ...
    for slug, section in class_sections:
        if (
            slug not in TARGET_SLUGS
            and slug not in DB5B_SLUGS
            and slug not in DB5A_SLUGS
        ):
            continue
        ...
        # Parse Passives
        passives_match = re.search(
            r"recommendedSkillSlugs:\s*\[(.*?)\]", section, re.DOTALL
        )
        if passives_match:
        ...
        if featured_start != -1:
            ...
            end_idx = (
                featured_start + 22 + next_block.start() if next_block else len(section)
            )
            ...
            for cat_id, slugs_str in blocks:
                category = (
                    "bm2"
                    if "battle-mode-2" in cat_id
                    else ("bm3" if "battle-mode-3" in cat_id else "buff")
                )
        ...
        if combo_start != -1:
            ...
            end_idx = (
                combo_start + 15 + next_block.start() if next_block else len(section)
            )
            combo_content = section[combo_start:end_idx]

            slug_lists = re.findall(
                r"skillSlugs:\s*\[([^\]]*)\]", combo_content, re.DOTALL
            )
        ...
        if slug == "force-blader":
            blade_buffs_match = re.search(
                r"bladeBuffConditionals:\s*\[(.*?)\]", section, re.DOTALL
            )
        ...
        rejected[slug] = rej
        coverage[slug] = {"mapped": len(skills), "rejected": len(rej)}
    ...
    expected_classes = 9
    coverage_percentage = (
        (total_classes / expected_classes) * 100 if expected_classes > 0 else 0
    )
    ...
    final_manifest = {
        "metadata": {
            ...
            "manifest_checksum": hashlib.md5(
                json.dumps(consolidated_manifest, sort_keys=True).encode("utf-8")
            ).hexdigest(),
        },
    ...
    if coverage_percentage >= 95 and critical_unresolved == 0:
        ...
        if critical_unresolved > 0:
            print(
                "Reason: There are unresolved or ambiguous skill aliases that must be fixed before DB6."
            )
