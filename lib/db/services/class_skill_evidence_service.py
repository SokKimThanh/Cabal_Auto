...
class ClassSkillEvidenceService:
    ...
    def map_category(self, title):
        ...
        if "battle mode 1" in title_lower or "bm1" in title_lower:
            return "bm1"
        if "battle mode 2" in title_lower or "bm2" in title_lower:
            return "bm2"
        if "battle mode 3" in title_lower or "bm3" in title_lower:
            return "bm3"
        if "debuff" in title_lower:
            return "debuff"
        if "buff" in title_lower:
            return "buff"
        if "passive" in title_lower:
            return "passive"
        if "heal" in title_lower:
            return "utility"
        if "combo" in title_lower:
            return "attack"
        if "totem" in title_lower:
            return "utility"
        return "attack"
    ...
    def extract_categories(self, content):
        ...
        for i in range(len(slugs_pos) - 1):
            ...
            section_matches = re.finditer(
                r'id:\s*"([^"]+)"\s*,\s*title:\s*"([^"]+)"[\s\S]*?skillSlugs:\s*\[(.*?)\]',
                chunk,
            )
            ...
            combo_match = re.search(
                r"comboSection:\s*\{[\s\S]*?skillSlugs:\s*\[(.*?)\]", chunk
            )
            if combo_match:
                skill_categories.append(
                    {
            ...
            passive_match = re.search(
                r"passiveSkillConfig:\s*\{[\s\S]*?recommendedSkillSlugs:\s*\[(.*?)\]",
                chunk,
            )
            if passive_match:
                skill_categories.append(
    ...
    def report(self, manifest):
        ...
        print(
            f"Forbidden inputs: skills.json, image-count-skill-db-cabal.txt, color-skill-character-db-cabal.txt"
        )
