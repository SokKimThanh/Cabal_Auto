...
class SeedBM3SynergiesService:
    def __init__(self, filepath: str = None):
        if filepath is None:
            self.filepath = os.getenv(
                "CABAL_BM3_SYNERGY_DB_FILE", "lib/data/bm2-bm3-skill-db-cabal.txt"
            )
        else:
    ...
    def parse_synergies(self) -> Tuple[List[Dict], int, int, str]:
        ...
        try:
            ...
            logging.error(
                f"[SeedBM3SynergiesService] Could not find data file at: {self.filepath}"
            )
            return [], 0, 0, ""
        ...
        class_blocks = re.split(
            r'\b"?([a-zA-Z_-]+)"?:\s*\{\s*id:\s*"bm3-synergies"', content
        )
        ...
        for i in range(1, len(class_blocks), 2):
            ...
            # Extract synergies
            synergies = re.finditer(
                r'synergyName:\s*"([^"]+)",\s*activationSequence:\s*"([^"]+)",\s*recommendation:\s*"([^"]+)",\s*effects:\s*\[(.*?)\]',
                rows_content,
                re.DOTALL,
            )
            ...
            for syn in synergies:
                ...
                effects_list = []
                effects = re.finditer(
                    r'stat:\s*"([^"]+)",\s*value:\s*"([^"]+)",\s*duration:\s*"([^"]+)",\s*target:\s*"([^"]+)"',
                    effects_str,
                )
                for eff in effects:
            ...
            if class_synergies:
                parsed_classes.append(
                    {"class_slug": class_slug, "synergies": class_synergies}
                )
    ...
    def seed(self) -> Tuple[bool, int, int]:
        ...
        """
        parsed_classes, source_synergy_count, source_effect_count, file_hash = (
            self.parse_synergies()
        )
        ...
        print(
            f"[SeedBM3SynergiesService] Parsed {source_synergy_count} synergies, {source_effect_count} effects for {len(parsed_classes)} classes."
        )
        print(f"Source ID: {self.source_id}")
        print(
            f"Parser Boundary: Exact regex matching on 'rows: [...]' within 'id: \"bm3-synergies\"' blocks."
        )
        print(
            f"Forbidden Inputs: User config files are ignored, only the exact parsed file is used."
        )

        conn, is_local = get_connection()
        if not conn:
            logging.error(
                "[SeedBM3SynergiesService] Could not get database connection."
            )
            return False, 0, 0
        ...
        try:
            ...
            for cls_data in parsed_classes:
                ...
                if class_id is None:
                    ...
                    logging.warning(
                        f"[SeedBM3SynergiesService] Unmatched class slug: {class_slug}"
                    )
                    continue
                ...
                for syn in cls_data["synergies"]:
                    ...
                    if existing_syn:
                        ...
                        cursor.execute(
                            "DELETE FROM synergy_effects WHERE synergy_id = ?",
                            (synergy_id,),
                        )
                    else:
                        cursor.execute(
            ...
            if unmatched_classes:
                print(
                    f"[SeedBM3SynergiesService] Unmatched class slugs: {unmatched_classes}"
                )
            ...
            if fk_issues:
                logging.error(
                    f"[SeedBM3SynergiesService] Foreign key violations: {fk_issues}"
                )
                # Report how many were processed before failing the check
                logging.error(
                    f"Rolling back after processing {seeded_synergies} synergies and {seeded_effects} effects."
                )
                conn.rollback()
                return False, 0, 0
            ...
            if orphans:
                logging.error(
                    f"[SeedBM3SynergiesService] Orphaned synergy effects found: {len(orphans)}"
                )
                logging.error(
                    f"Rolling back after processing {seeded_synergies} synergies and {seeded_effects} effects."
                )
