...
def run_audit(content=None):
    ...
    for slug, section in class_sections:
        ...
        coverage[slug] = {
            "total": len(skills),
            "passives": len(
                [s for s, data in skills.items() if data["category"] == "passive"]
            ),
            "attacks": len(
                [s for s, data in skills.items() if data["category"] == "attack"]
            ),
            "bm2": len([s for s, data in skills.items() if data["category"] == "bm2"]),
            "bm3": len([s for s, data in skills.items() if data["category"] == "bm3"]),
            "buffs": len(
                [s for s, data in skills.items() if data["category"] == "buff"]
            ),
            "rejected": len(rej),
        }
    ...
    print(
        "Parser Boundary: slug sections (wizard, force-archer, force-gunner, dark-mage)"
    )
