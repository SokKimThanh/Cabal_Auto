...
- Phương thức `wait_for_hit_zone(screen_capture, timeout_sec: float = None, is_target_alive_check: callable = None) -> bool`:
  ...
  - Nếu phát hiện vạch sáng (HSV Value > 210 — giá trị khởi điểm cần hiệu chỉnh thực nghiệm theo theme UI/độ sáng màn hình của client, không coi là hằng số tuyệt đối đúng mọi cấu hình):
    ...
    - Gọi `mark_cast()` trên `SkillRuntime` ngay sau khi bắn phím thành công, theo đúng quy tắc đã chốt ở CB3C (Combo Mode dùng `SkillRuntime` làm nguồn sự thật duy nhất cho vị trí rotation — `wait_for_hit_zone()` không tự ý coi việc bắn trúng hit-zone là đã tiến rotation, phải đi qua `mark_cast()`).
...
- Trước khi gửi phím skill tiếp theo trong chuỗi xoay tua, gọi `wait_for_hit_zone()` (truyền kèm `is_target_alive_check` để hỗ trợ ngắt sớm) để căn đúng nhịp vạch vào vùng 2 vạch, và gọi `mark_cast()` theo đúng luồng ở mục 2.
...
- (Added) Test `mark_cast()` được gọi đúng một lần mỗi lần bắn trúng hit-zone, và không có nơi nào khác tự ý tăng rotation index trong Combo Mode (đối chiếu quy tắc CB3C).
...
- `mark_cast()` được gọi đúng theo quy tắc sở hữu rotation index của CB3C.
