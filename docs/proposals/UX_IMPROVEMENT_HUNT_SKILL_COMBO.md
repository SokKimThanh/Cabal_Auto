...
- **Thêm Cột Visual Success Rate:**
  - Bỏ cột số Text "%", thay bằng một thanh tiến trình mini (Mini Bar) ngay trong cột của `Treeview` (bằng cách dùng custom style hoặc chuyển sang Frame-based list thay vì `ttk.Treeview`).
